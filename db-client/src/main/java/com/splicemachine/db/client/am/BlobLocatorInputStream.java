...
class BlobLocatorInputStream extends java.io.InputStream{
     ...
     */
    BlobLocatorInputStream(Connection connection,Blob blob) throws SqlException{
     ...
     *                      length of the <code>Blob</code>.
     */
    BlobLocatorInputStream(Connection connection,Blob blob, long position,long length) throws SqlException{
        if(SanityManager.DEBUG){
            SanityManager.ASSERT(blob.isLocator());
        }
        this.connection=connection;
