...
public class Statement implements java.sql.Statement, StatementCallbackInterface,
    ...
    public Statement(Agent agent, ClientConnection connection) throws SqlException {
        this();
        initStatement(agent, connection);
    }

    public void resetStatement(Agent agent, ClientConnection connection) throws SqlException {
        resetStatement();
        initStatement(agent, connection);
    }
    ...
    public Statement(Agent agent, ClientConnection connection, int type, int concurrency, int holdability,
    ...
    public void resetStatement(Agent agent, ClientConnection connection, int type, int concurrency, int holdability,
                               int autoGeneratedKeys, String[] columnNames, int[] columnIndexes) throws SqlException {
    ...
    private void initStatement(int type, int concurrency, int holdability,
        ...
        default:
            throw new SqlException(agent_.logWriter_, 
                new ClientMessageId(SQLState.INVALID_API_PARAMETER),
                    Integer.valueOf(type), "type", "createStatement()");
        }
        ...
        default:
            throw new SqlException(agent_.logWriter_, 
                new ClientMessageId(SQLState.INVALID_API_PARAMETER),
                    Integer.valueOf(concurrency), "concurrency",
                "createStatement()");
        }
        ...
        default:
            throw new SqlException(agent_.logWriter_,
                new ClientMessageId(SQLState.INVALID_API_PARAMETER),
                    Integer.valueOf(holdability), "holdability",
                "createStatement()");
        }
    ...
    public String getProgress() {
        ...
        try (ClientConnection sideConnection = connection_.getSideConnection();
             ...
             java.sql.ResultSet rs = s.executeQuery("call SYSCS_UTIL.SYSCS_GET_PROGRESS()") ) {
            while(rs.next()) {
                ...
                if (rows < 0 || (maxRows_ != 0 && rows > maxRows_)) {
                    throw new SqlException(agent_.logWriter_, 
                        new ClientMessageId(SQLState.INVALID_ST_FETCH_SIZE),
                            Integer.valueOf(rows)).getSQLException();
                }
                fetchSize_ = rows;
            }
        }
    ...
    private boolean willTickleServer(int number, boolean allowAutoCommits) throws SqlException {
        ...
        if (connection_.autoCommit_ && requiresAutocommit) { // for the auto-commit;
            if (connection_.isXAConnection_) {
                return (connection_.getXAState() == ClientConnection.XA_T0_NOT_ASSOCIATED) ;
            } else {
                return true;
            }
        }
            ...
            if (resultSet_ != null && resultSet_.resultSetHoldability_ != resultSetHoldability_ && sqlMode_ != isCall__) {
                throw new SqlException(agent_.logWriter_, 
                    new ClientMessageId(SQLState.UNABLE_TO_OPEN_RS_WITH_REQUESTED_HOLDABILITY),
                        Integer.valueOf(resultSetHoldability_));
            }
    ...
    void checkAutoGeneratedKeysParameters() throws SqlException {
        if (autoGeneratedKeys_ != java.sql.Statement.NO_GENERATED_KEYS &&
            ...
            throw new SqlException(agent_.logWriter_,
                new ClientMessageId(SQLState.INVALID_API_PARAMETER),
                    Integer.valueOf(autoGeneratedKeys_), "autoGeneratedKeys",
        ...
        if (generatedKeysColumnNames_ != null && 
            ...
            throw new SqlException(agent_.logWriter_,
                    ...
                    Integer.valueOf(generatedKeysColumnNames_.length));
            }
        ...
        if (generatedKeysColumnIndexes_ != null && 
            ...
            throw new SqlException(agent_.logWriter_,
                    new ClientMessageId(SQLState.INVALID_COLUMN_ARRAY_LENGTH),
                    Integer.valueOf(generatedKeysColumnIndexes_.length));
            }
        }
