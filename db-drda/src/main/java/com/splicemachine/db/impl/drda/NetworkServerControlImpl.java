...
*/
@SuppressFBWarnings(value="IS2_INCONSISTENT_SYNC", justification = "TOFIX: DB-10437")
public final class NetworkServerControlImpl {
    ...
    private final static String DRDA_PROP_MESSAGES = "com.splicemachine.db.loc.drda.messages";
    private final static String DRDA_PROP_DEBUG = "derby.drda.debug";
    private final static String CLOUDSCAPE_DRIVER = "com.splicemachine.db.jdbc.EmbeddedDriver";
    ...
    // open sessions
    private final Hashtable sessionTable = new Hashtable();
    ...
    {
        if (out != null)
            return new PrintWriter(new OutputStreamWriter(out, StandardCharsets.UTF_8), true /* flush the buffer at the end of each line */);
        else
            return null;
    }
     ...
     * and possibly db.log.  To turn off logging the errors to the console,
        ...
        throws Exception
    {
        ...
        try {
            ...
            setLogWriter(new PrintWriter(new OutputStreamWriter(silentStream, StandardCharsets.UTF_8)));
    ...
    */
        ...
        throws Exception
    {
        ...
        else
        {
            ...
            args[1] = Integer.toString(connNum);
     ...
     * written to db.log each time a connection is made.
     *
    ...
    public void netSetMaxThreads(int max) throws Exception
    {
        try {
            ...
            readResult();
            int newval = readInt();
            consolePropertyMessage("DRDA_MaxThreadsChange.I", Integer.toString(newval));
        } finally {
            closeSocket();
        }
        ...
        Session session) throws Throwable
    {
        try {
            ...
            if (version <= 0 || version > MAX_ALLOWED_PROTOCOL_VERSION)
                throw new Throwable(langUtil.getTextMessage("DRDA_UnknownProtocol.S", Integer.toString(version)));
     ...
     */
    private String getNetSysInfo() throws UnsupportedEncodingException {
        StringBuilder sysinfo = new StringBuilder();
        LocalizedResource localLangUtil = langUtil;
        if (currentSession != null && currentSession.langUtil != null)
        localLangUtil = currentSession.langUtil;
        sysinfo.append(localLangUtil.getTextMessage("DRDA_SysInfoBanner.I")).append("\n");
        sysinfo.append(localLangUtil.getTextMessage("DRDA_SysInfoVersion.I")).append(" ").append(att_srvrlslv);
        sysinfo.append("  ");
        sysinfo.append(localLangUtil.getTextMessage("DRDA_SysInfoBuild.I")).append(" ").append(buildNumber);
        sysinfo.append("  ");
        sysinfo.append(localLangUtil.getTextMessage("DRDA_SysInfoDrdaPRDID.I")).append(" ").append(prdId);
        if (SanityManager.DEBUG)
        {
        ...
        p.list(ps);
        sysinfo.append(bos.toString("UTF-8"));
...
//                                            s1.startHandshake();
    ...
    private void  getPropertyInfo() throws Exception
    {
        ...
        //If db.system.home is not set, directory will be null and trace files will get
    ...
    private boolean setTrace(boolean on)
    {
        ...
        {
            Session session = (Session) sessionTable.get(sessionArg);
            if (session != null)
            {
                if (on)
                    try {
    ...
    public String buildRuntimeInfo(LocalizedResource locallangUtil)
    {
        ...
        StringBuilder s = new StringBuilder(locallangUtil.getTextMessage("DRDA_RuntimeInfoBanner.I") + "\n");
        int sessionCount = 0;
        s.append(locallangUtil.getTextMessage("DRDA_RuntimeInfoSessionBanner.I")).append("\n");
        for (int i = 0; i < threadList.size(); i++)
        {
            ...
            {
                sessionCount ++;
                s.append(sessionInfo).append("\n");
            }
        }
        int waitingSessions = 0;
        for (int i = 0; i < runQueue.size(); i++)
        {
                s.append(((Session) runQueue.get(i)).buildRuntimeInfo("", locallangUtil));
                waitingSessions ++;
        }
        s.append("-------------------------------------------------------------\n");
        s.append(locallangUtil.getTextMessage("DRDA_RuntimeInfoNumThreads.I")).append(threadList.size()).append("\n");
        s.append(locallangUtil.getTextMessage("DRDA_RuntimeInfoNumActiveSessions.I")).append(sessionCount).append("\n");
        s.append(locallangUtil.getTextMessage("DRDA_RuntimeInfoNumWaitingSessions.I")).append(+waitingSessions).append("\n\n");

        Runtime rt = Runtime.getRuntime();
        rt.gc();
        long totalmem = rt.totalMemory();
        long freemem = rt.freeMemory();
        s.append(locallangUtil.getTextMessage("DRDA_RuntimeInfoTotalMemory.I")).append(+totalmem).append("\t");
        s.append(locallangUtil.getTextMessage("DRDA_RuntimeInfoFreeMemory.I")).append(+freemem).append("\n\n");
