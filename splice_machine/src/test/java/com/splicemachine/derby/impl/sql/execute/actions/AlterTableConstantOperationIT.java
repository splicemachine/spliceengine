...
public class AlterTableConstantOperationIT extends SpliceUnitTest {
    ...
    public void testAlterTableIsolationInactiveTransaction() throws Exception{
        ...
        try(Statement statement=connection2.createStatement()){
            try(ResultSet resultSet=statement.executeQuery(String.format("select * from %s",this.getTableReference(TABLE_NAME_4)))){
                Assert.assertEquals("Read Committed Violated",0,resultSetSize(resultSet));
                connection2.commit();
                Assert.assertEquals("Didn't expect to see another column until after commit.",4,columnWidth(resultSet));
            }
        }
        try(Statement s1 = connection1.createStatement()){
            s1.execute(String.format("alter table %s add column date timestamp",this.getTableReference(TABLE_NAME_4)));
            s1.execute(String.format("insert into %1$s (TaskId, empId, StartedAt, FinishedAt, Date) values (%2$d,'JC',09%3$d,09%4$d, '2013-05-28 13:03:20')",
                            this.getTableReference(TABLE_NAME_4),1244,0,30));
            try(ResultSet resultSet = s1.executeQuery(String.format("select * from %s", this.getTableReference(TABLE_NAME_4)))){
                Assert.assertEquals("Can't read own write", 1, resultSetSize(resultSet));
            }
        }
        connection1.commit();

        try(Statement s2 = connection2.createStatement()){
            try(ResultSet resultSet = s2.executeQuery(String.format("select * from %s", this.getTableReference(TABLE_NAME_4)))){
                Assert.assertEquals("Expected to see an additional row.",1,resultSetSize(resultSet));
                Assert.assertEquals("Expected to see another column.",5,columnWidth(resultSet));
            }
        }
