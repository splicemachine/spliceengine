...
public class GroupByOrderByIT {
    ...
    protected static SpliceTableWatcher t2Watcher = new SpliceTableWatcher("BAR", schemaWatcher.schemaName,
                                                                           "(A2 VARCHAR(3) NOT NULL, B2 INT NOT NULL)");
    private static List<String> t1Values = Arrays.asList(
            ...
            "('E4','P4',40)",
            "('E4','P5',80)");
    private static List<String> t2Values = Arrays.asList(
            "('F1', 10)",
            "('F2', 20)",
            "('F3', 30)",
            "('F4', 40)",
    ...
    public static TestRule chain = RuleChain.outerRule(spliceClassWatcher)
            ...
            .around(new SpliceDataWatcher() {
                ...
                protected void starting(Description description) {
                    try {
                        // load t2
                        for (String rowVal : t2Values) {
                            spliceClassWatcher.getStatement().executeUpdate("insert into " + t2Watcher.toString() + " values " + rowVal);
                        }
                    } catch (Exception e) {
                        throw new RuntimeException(e);
                    } finally {
    ...
    private static int verifyColumns(ResultSet rs, List<String> expectedColNames, List<String> excludedColNames) throws Exception {
    ...
    public void testSelectAllColumns() throws Exception {
        try (ResultSet rs = methodWatcher.executeQuery(
            ...
            Assert.assertEquals(12, verifyColumns(rs, Arrays.asList("EMPNUM", "PNUM", "HOURS"), Arrays.asList("HOURS")));
        }
    }
    ...
    public void testSelectAllColumnsGroupBySum() throws Exception {
        try (ResultSet rs = methodWatcher.executeQuery(
            ...
            Assert.assertEquals(12, verifyColumns(rs, Arrays.asList("EMPNUM", "PNUM", "SUM_HOURS"), Arrays.asList("SUM_HOURS")));
        }
        /* test group by position number */
        try (ResultSet rs = methodWatcher.executeQuery(
    ...
    @Test
    public void testNonCorrelatedSubqueryAggregateGroupByOrderBy() throws Exception {
        try (ResultSet rs = methodWatcher.executeQuery(
                String.format("SELECT EMPNUM, (SELECT MAX(B2) FROM %s) FROM %s GROUP BY EMPNUM ORDER BY EMPNUM", t2Watcher.toString(), t1Watcher.toString()))) {
            Assert.assertTrue(rs.next());
            Assert.assertEquals("E1", rs.getString(1));
            Assert.assertEquals(70, rs.getInt(2));
            Assert.assertTrue(rs.next());
            Assert.assertEquals("E2", rs.getString(1));
            Assert.assertEquals(70, rs.getInt(2));
            Assert.assertTrue(rs.next());
            Assert.assertEquals("E3", rs.getString(1));
            Assert.assertEquals(70, rs.getInt(2));
            Assert.assertTrue(rs.next());
            Assert.assertEquals("E4", rs.getString(1));
            Assert.assertEquals(70, rs.getInt(2));
