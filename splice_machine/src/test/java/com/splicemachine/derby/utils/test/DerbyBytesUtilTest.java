...
import static com.splicemachine.db.impl.sql.catalog.SYSTABLESRowFactory.CURRENT_TABLE_VERSION;

@Category(ArchitectureIndependent.class)
public class DerbyBytesUtilTest{
    protected static J2SEDataValueFactory dvf=new J2SEDataValueFactory();
    private static final String tableVersion = CURRENT_TABLE_VERSION;
    ...
    public void generateIndexKeyTest() throws Exception{
        ...
        byte[] indexKey1=DerbyBytesUtil.generateIndexKey(
                generateDataValueDescriptors("John","Leach",1l),null,tableVersion,false);
        byte[] indexKey2=DerbyBytesUtil.generateIndexKey(
                generateDataValueDescriptors("John","Leach",2l),null,tableVersion,false);
        ...
        byte[] indexKey4=DerbyBytesUtil.generateIndexKey(
                generateDataValueDescriptors("Monte","Alpine"),null,tableVersion,false);
        byte[] indexKey5=DerbyBytesUtil.generateScanKeyForIndex(
                generateDataValueDescriptors("John","Leach",1l),1,null,tableVersion,false);
    ...
    public void generateScanKeyForStringIndex() throws IOException,
        ...
        byte[] correct=DerbyBytesUtil.generateIndexKey(descriptors,null,tableVersion,false);
        ByteComparator byteComparator=ByteComparisons.comparator();
        Assert.assertTrue(byteComparator.compare(correct,
                DerbyBytesUtil.generateScanKeyForIndex(descriptors,ScanController.GE,null,tableVersion,false))==0);
        Assert.assertTrue(byteComparator.compare(correct,DerbyBytesUtil.generateScanKeyForIndex(descriptors,ScanController.GT,null,tableVersion,false))<0);
        Assert.assertTrue(byteComparator.compare(correct,
                DerbyBytesUtil.generateScanKeyForIndex(descriptors,ScanController.GE,null,tableVersion,false))==0);
        Assert.assertTrue(byteComparator.compare(correct,
...
//	}
    ...
    public void testIndexGenerationForHBaseRowLocationTestDeepCopy()
        ...
        byte[] beginKey=DerbyBytesUtil.generateScanKeyForIndex(descArray,1,null,tableVersion,false);
        byte[] endKey=DerbyBytesUtil.generateScanKeyForIndex(descArray,-1,null,tableVersion,false);
        byte[] baseline=DerbyBytesUtil.generateIndexKey(descArray,null,tableVersion,false);
