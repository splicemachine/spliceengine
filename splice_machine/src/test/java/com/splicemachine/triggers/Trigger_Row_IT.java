...
public class Trigger_Row_IT {
    ...
    public void afterInsertCascadingTriggers() throws Exception {
        ...
        Assert.assertEquals(1,methodWatcher.query("select a from cascade1"));

        // then -- verify content of table 2
        Assert.assertEquals(1,methodWatcher.query("select a from cascade2"));
        Assert.assertEquals("cascade1_insert",methodWatcher.query("select name from cascade2"));
        Timestamp ts2 = methodWatcher.query("select t from cascade2");

        // then -- verify content of table 3
        Assert.assertEquals(1,methodWatcher.query("select a from cascade3"));
        Assert.assertEquals("cascade2_insert",methodWatcher.query("select name from cascade3"));
