...
public abstract class ForwardingDataSetProcessor implements DataSetProcessor{
    ...
    @Override
    public <V> DataSet<ExecRow> readTextFile(SpliceOperation op, String location, int[] baseColumnMap, OperationContext context, Qualifier[][] qualifiers, DataValueDescriptor probeValue, ExecRow execRow,
                                             boolean useSample, double sampleFraction, CsvOptions csvOptions) throws StandardException {
        return delegate.readTextFile(op, location, baseColumnMap, context,  qualifiers, probeValue, execRow, useSample, sampleFraction, csvOptions);
    }
