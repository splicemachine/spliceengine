...
public class ControlDataSetProcessor implements DataSetProcessor{
    ...
    @Override
    public <V> DataSet<ExecRow> readTextFile(SpliceOperation op, String location, int[] baseColumnMap,
        ...
        DistributedDataSetProcessor proc = EngineDriver.driver().processorFactory().distributedProcessor();
        return new ControlDataSet(proc.readTextFile(op,location, baseColumnMap, context, qualifiers, probeValue, execRow,
                useSample, sampleFraction, csvOptions).toLocalIterator());
    }
