...
public class TieredTaskScheduler<T extends Task> implements TaskScheduler<T>{
    ...
    private static final Logger LOG=Logger.getLogger(TieredTaskScheduler.class);
    ...
    private final WorkStealingTaskScheduler<T> maintenanceScheduler;
    ...
    public TieredTaskScheduler(TieredTaskSchedulerSetup schedulerSetup,
        ...
        int maintenanceThreads = schedulerSetup.maxMaintenanceThreads();
        long pollTime=schedulerSetup.pollTimeForPriority(priorityLevels[priorityLevels.length-1]);
        maintenanceScheduler=new WorkStealingTaskScheduler<>(maintenanceThreads,
                pollTime,
                comparator,
                priorityList,
                "maintenance-tier-"+100);
    }
    ...
    public TaskFuture submit(T task) throws ExecutionException{
        if(task.isMaintenanceTask()){
            return maintenanceScheduler.submit(task);
        }
        ...
        if(LOG.isTraceEnabled())
            LOG.trace("Attempting to submit task "+task+" to tier "+levelTier.priorityLevel);
        TaskFuture future=levelTier.scheduler.tryExecute(task);
        if(future!=null) return future; //successfully submitted

        if(LOG.isTraceEnabled())
            LOG.trace("Submission to base level failed, attempting to shrug ["+ task+"] to lower priority");
    ...
    private TaskFuture overflow(T task,Tier baseTier) throws ExecutionException{
				 ...
				 */
        if(LOG.isTraceEnabled())
            LOG.trace("Overflowing task "+task+" from tier "+baseTier);
