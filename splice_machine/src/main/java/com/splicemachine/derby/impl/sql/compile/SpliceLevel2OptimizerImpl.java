 ...
 */
public class SpliceLevel2OptimizerImpl implements Optimizer {
    private static final Logger LOG = Logger.getLogger(SpliceLevel2OptimizerImpl.class);
	...
	protected RowOrdering currentRowOrdering = new SpliceRowOrderingImpl();
	protected RowOrdering bestRowOrdering = new SpliceRowOrderingImpl();
	...
	public SpliceLevel2OptimizerImpl(OptimizableList optimizableList, 
		...
		SpliceLogUtils.debug(LOG, "SpliceLevel2OptimizerImpl optimizableList=%s, predicateList=%s, joinStrategies=%s, requiredRowOrdering=%s"
				+ "numTablesInQuery%d",optimizableList,predicateList,Arrays.toString(joinStrategies),requiredRowOrdering,numTablesInQuery);
	...
	public void trace(int traceFlag, int intParam1, int intParam2, double doubleParam, Object objectParam1) {
		...
		switch (traceFlag) {
			...
			case REMEMBERING_BEST_ACCESS_PATH:
				Tracer.traceBestAccess(objectParam1);
				traceString = 
	...
	private String lockModeThreshold( // TODO REMOVE JL
	...
	public void prepForNextRound() {
		SpliceLogUtils.trace(LOG, "prepForNextRound");
	...
	public boolean getNextPermutation() throws StandardException {
		SpliceLogUtils.trace(LOG, "getNextPermutation");
	...
	*/
				...
				throws StandardException
	{
		...
		retval =  curOpt.nextAccessPath(this,
										...
										currentRowOrdering);
		//System.out.println("current ---" + curOpt.getCurrentAccessPath());
