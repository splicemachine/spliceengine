...
public abstract class SpliceController implements ConglomerateController{
    ...
    public boolean delete(RowLocation loc) throws StandardException{
        ...
        try{
            ...
            if (htable.isRedoPartition()) {
                DataPut put = opFactory.newDataPut(trans.getActiveStateTxn(), loc.getBytes());
                baseOperationFactory.setTemplate(put,new ValueRow(0));
                int numberOfColumns = openSpliceConglomerate.cloneExecRowTemplate().nColumns();
                ...
                baseOperationFactory.setTemplate(put,openSpliceConglomerate.cloneExecRowTemplate());
    ...
    public boolean batchFetch(List<RowLocation> locations, List<ExecRow> destRows,FormatableBitSet validColumns,boolean waitForLock) throws StandardException{
        if (locations.isEmpty())
            return false;
        if (validColumns == null) { // Grab all
            validColumns = new FormatableBitSet(destRows.get(0).nColumns());
            validColumns.setAll();
        }
        Partition htable = getTable();
        ...
        try{
            ...
            DataGet get = createGet(baseGet,destRows.get(0).getRowArray(),validColumns);
            SpliceQuery spliceQuery = new SpliceQuery(destRows.get(0),validColumns);
            ...
            if (htable.isRedoPartition()) {
                ...
                while (results.hasNext()) {
                    ...
                    if (result == null || result.size() == 0) {
                        throw new UnsupportedOperationException("Not Correct ...");
                    }
    ...
    protected void encodeRow(Partition table, TxnView txnView, DataValueDescriptor[] row, DataPut put, FormatableBitSet validColumns) throws StandardException, IOException {
        try {
            if (table.isRedoPartition()) {
                ...
                baseOperationFactory.setTemplate(put,rowTemplate);
                put.addCell(SIConstants.DEFAULT_FAMILY_ACTIVE_BYTES, SIConstants.PACKED_COLUMN_BYTES, 1, record.getValue());
            } else {
                if (entryEncoder == null) {
