...
public class AlterTableWriteFactory implements LocalWriteFactory{
    ...
    public static AlterTableWriteFactory create(DDLMessage.DDLChange ddlChange,TransactionReadController readController) {
        DDLMessage.DDLChangeType changeType = ddlChange.getDdlChangeType();
        if (changeType == DDLMessage.DDLChangeType.ADD_COLUMN)
            return new AlterTableWriteFactory(ddlChange,new TentativeAddColumnDesc(ddlChange.getTentativeAddColumn()),readController);
        else if (changeType == DDLMessage.DDLChangeType.ADD_UNIQUE_CONSTRAINT)
            return new AlterTableWriteFactory(ddlChange,new TentativeAddConstraintDesc(ddlChange.getTentativeAddConstraint()),readController);
        else if (changeType == DDLMessage.DDLChangeType.DROP_PRIMARY_KEY)
            return new AlterTableWriteFactory(ddlChange,new TentativeDropPKConstraintDesc(ddlChange.getTentativeDropPKConstraint()),readController);
        else if (changeType == DDLMessage.DDLChangeType.DROP_COLUMN)
            return new AlterTableWriteFactory(ddlChange,new TentativeDropColumnDesc(ddlChange.getTentativeDropColumn()),readController);
        else {
