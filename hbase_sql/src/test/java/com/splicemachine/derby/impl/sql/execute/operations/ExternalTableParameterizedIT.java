...
public class ExternalTableParameterizedIT extends SpliceUnitTest {

    private static final String SCHEMA_NAME = ExternalTableParameterizedIT.class.getSimpleName().toUpperCase();
    ...
    public static Collection<Object[]> data() {
        ...
        params.add(new Object[]{"PARQUET"});
        params.add(new Object[]{"ORC"});
        params.add(new Object[]{"AVRO"});
        return params;
    }
    public ExternalTableParameterizedIT(String format)
    {
        fileFormats = new String[]{format};
    }
    ...
    public void testCollectStats() throws Exception {
        for( String fileFormat : new String[]{"ORC", "PARQUET", "AVRO", "TEXTFILE"}) {
            ...
            rs = ps.executeQuery();
            rs.next();
            Assert.assertEquals("Error with COLLECT_TABLE_STATISTICS for external table",SCHEMA_NAME,  rs.getString(1));
            rs.close();

            ResultSet rs2 = methodWatcher.executeQuery("select total_row_count from sysvw.systablestatistics where " +
                    "schemaname = '" + SCHEMA_NAME + "' and tablename = '" + name + "'");
            ...
            spliceClassWatcher.executeUpdate("CALL  SYSCS_UTIL.DROP_TABLE_STATISTICS ('" + SCHEMA_NAME +"', '" + name + "')");
            ...
            rs = ps.executeQuery();
            rs.next();
            Assert.assertEquals("Error with COLLECT_SCHEMA_STATISTICS for external table",SCHEMA_NAME,  rs.getString(1));
            rs.close();
            ...
            spliceClassWatcher.executeUpdate("CALL  SYSCS_UTIL.DROP_SCHEMA_STATISTICS ('" + SCHEMA_NAME + "')");

            // make sure it is clean
            rs2 = methodWatcher.executeQuery("select total_row_count from sysvw.systablestatistics where schemaname = '" + SCHEMA_NAME + "' ");
            Assert.assertEquals("", TestUtils.FormattedResult.ResultFactory.toString(rs2));
            rs2.close();
        }
    }
    ...
    public void testPartitionPruning() throws Exception {
        ...
        ResultSet rs2 = methodWatcher.executeQuery("select total_row_count from sysvw.systablestatistics where schemaname = '" + SCHEMA_NAME + "' and tablename = 'PARTITION_PRUNE_ORC'");
        String expected = "TOTAL_ROW_COUNT |\n" +
                "------------------\n" +
                "        3        |";
        Assert.assertEquals(expected, TestUtils.FormattedResult.ResultFactory.toString(rs2));
    ...
    public void testCollectAvroDateStats() throws Exception {
        ...
        ResultSet rs2 = methodWatcher.executeQuery("select total_row_count from sysvw.systablestatistics where schemaname = '" + SCHEMA_NAME + "' and tablename = 'AVRO_DATE_STATS'");
        String expected = "TOTAL_ROW_COUNT |\n" +
                "------------------\n" +
                "        4        |";
        Assert.assertEquals(expected, TestUtils.FormattedResult.ResultFactory.toString(rs2));
    ...
    public void testShowCreateTableTextExternalTable() throws Exception {
        ...
        methodWatcher.execute(String.format("CREATE EXTERNAL TABLE myTextTable\n" +
                ...
                "                    STORED AS TEXTFILE\n" +
                "                    LOCATION '%s'", tablePath));
        ResultSet rs = methodWatcher.executeQuery("CALL SYSCS_UTIL.SHOW_CREATE_TABLE('" + SCHEMA_NAME + "','MYTEXTTABLE')");
        rs.next();
        Assert.assertEquals("CREATE EXTERNAL TABLE \"" + SCHEMA_NAME + "\".\"MYTEXTTABLE\" (\n" +
                "\"COL1\" INTEGER\n" +
                ",\"COL2\" VARCHAR(24)\n" +
                ") \n" +
                "PARTITIONED BY (\"COL1\")\n" +
    ...
    public void testShowCreateTableExternalTable() throws Exception {
        for( String fileFormat : new String[]{"ORC", "PARQUET", "AVRO", "TEXTFILE"}) {
            ...
            methodWatcher.execute(String.format("CREATE EXTERNAL TABLE " + name + "\n" +
                    ...
                    "                    LOCATION '%s'", tablePath));
            ResultSet rs = methodWatcher.executeQuery("CALL SYSCS_UTIL.SHOW_CREATE_TABLE('" + SCHEMA_NAME + "','" + name + "')");
            rs.next();
            Assert.assertEquals("CREATE EXTERNAL TABLE \"" + SCHEMA_NAME + "\".\"" + name + "\" (\n" +
                    "\"COL1\" INTEGER\n" +
                    ",\"COL2\" VARCHAR(24)\n" +
                    ") \n" +
                    "PARTITIONED BY (\"COL1\")\n" +
    ...
    public void testShowCreateTableSnappyParquetExternalTable() throws Exception {
        ...
        methodWatcher.execute(String.format("CREATE EXTERNAL TABLE mySnappyParquetTable\n" +
                ...
                "                    STORED AS PARQUET\n" +
                "                    LOCATION '%s'", tablePath));
        ResultSet rs = methodWatcher.executeQuery("CALL SYSCS_UTIL.SHOW_CREATE_TABLE('" + SCHEMA_NAME + "','MYSNAPPYPARQUETTABLE')");
        rs.next();
        Assert.assertEquals("CREATE EXTERNAL TABLE \"" + SCHEMA_NAME + "\".\"MYSNAPPYPARQUETTABLE\" (\n" +
                "\"COL1\" INTEGER\n" +
                ",\"COL2\" VARCHAR(24)\n" +
                ",\"COL3\" INTEGER\n" +
                ") \n" +
