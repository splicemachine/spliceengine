...
public class ExternalTableIT extends SpliceUnitTest{
    ...
    @Test
    public void testScalarPartitionConstraint() throws Exception {
        try {
            methodWatcher.executeUpdate("create external table table_numerical_partition (col1 int, col2 date)" +
                    "PARTITIONED BY (col2) STORED AS PARQUET LOCATION 'HUMPTY_DUMPTY_MOLITOR'");
            Assert.fail("Exception not thrown");
        } catch (SQLException e) {
