...
public class StreamableRDDTest_Failures extends BaseStreamTest implements Serializable {
    ...
    public void testBasicStream() throws Exception {
        ...
        StreamableRDD srdd = new StreamableRDD(rdd.values(), sl.getUuid(), hostAndPort.getHost(), hostAndPort.getPort());
        srdd.submit();
    ...
    public void testBlockingMedium() throws StandardException, FileNotFoundException, UnsupportedEncodingException {
        ...
        final StreamableRDD srdd = new StreamableRDD(rdd.values(), null, sl.getUuid(), hostAndPort.getHost(), hostAndPort.getPort(), batches, batchSize);
        new Thread() {
            @Override
            public void run() {
                try {
    ...
    public void testFailureBeforeLargeOffset() throws StandardException {
        ...
        final StreamableRDD srdd = new StreamableRDD(rdd.values(), sl.getUuid(), hostAndPort.getHost(), hostAndPort.getPort());
        new Thread() {
            @Override
            public void run() {
                try {
    ...
    public void testPersistentFailureWithOffset() throws StandardException, FileNotFoundException, UnsupportedEncodingException {
        ...
        final StreamableRDD srdd = new StreamableRDD(rdd.distinct().values(), null, sl.getUuid(), hostAndPort.getHost(), hostAndPort.getPort(), batches, batchSize);
        new Thread() {
            @Override
            public void run() {
                try {
