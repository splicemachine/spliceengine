...
public class SparkHBaseBulkImport implements HBaseBulkImporter{
        ...
        if (outputKeysOnly) {
            ...
            for (ConglomerateDescriptor searchCD :list) {
                if (searchCD.isIndex() && !searchCD.isPrimaryKey() &&
                        (indexName != null && searchCD.getObjectName().compareToIgnoreCase(indexName) == 0)) {
    ...
    private void partitionUsingRDDSortUsingDataFrame(List<BulkImportPartition> bulkImportPartitions,
        ...
        SQLContext sqlContext = new SQLContext(SpliceSpark.getContext());
        StructType schema = createSchema();
        DataFrame rowAndIndexesDataFrame =
                sqlContext.createDataFrame(javaRowRdd, schema);

        DataFrame partitionAndSorted =  rowAndIndexesDataFrame
        ...
        RDD<String> HFileSet = partitionAndSorted.mapPartitions(writer, tag);
