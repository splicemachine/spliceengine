    ...
    public DataSet<V> buildDataSet(Object caller) throws StandardException {
        ...
        if (storedAs!= null) {
            ...
            if (storedAs.equals("T"))
                ...
                locatedRows = dsp.readParquetFile(schema, baseColumnMap,partitionByColumns,location,operationContext,qualifiers,null,execRow, useSample, sampleFraction, mergeSchema);
            else if (storedAs.equals("A")) {
                ExternalTableUtils.supportAvroDateTypeColumns(execRow);
                locatedRows = dsp.readAvroFile(schema, baseColumnMap, partitionByColumns, location, operationContext, qualifiers, null, execRow, useSample, sampleFraction, mergeSchema);
            }
