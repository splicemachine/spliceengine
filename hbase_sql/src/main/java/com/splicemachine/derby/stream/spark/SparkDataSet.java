...
public class SparkDataSet<V> implements DataSet<V> {
    ...
    public SparkDataSet(Dataset<Row> rdd, String rddname) {
        this.rdd = rdd;
//        if (rdd != null && rddname != null) this.rdd.(rddname);
    }
...
//        }
    ...
    public <Op extends SpliceOperation, U> DataSet<U> mapPartitions(SpliceFlatMapFunction<Op,Iterator<V>, U> f) {
        try {
            ...
            return new SparkDataSet<U>(toSparkRow(SparkDataSet.<V>toSpliceLocatedRow(rdd, context).mapPartitions(new SparkFlatMapFunction<>(f)), context),f.getSparkName());
        } catch (Exception e) {
            throw Exceptions.throwAsRuntime(e);
        }
    }
...
//        }
        try {
            OperationContext<Op> context = f.operationContext;
            return new SparkDataSet<U>(toSparkRow(SparkDataSet.<V>toSpliceLocatedRow(rdd, context).map(new SparkSpliceFunctionWrapper<>(f)), context),f.getSparkName());
        } catch (Exception e) {
            throw Exceptions.throwAsRuntime(e);
        }
    }
...
//        }
        ...
        try {
            OperationContext<Op> context = f.operationContext;
            return new SparkDataSet<V>(toSparkRow(SparkDataSet.<V>toSpliceLocatedRow(rdd, context).filter(new SparkSpliceFunctionWrapper<>(f)), context),f.getSparkName());
        } catch (Exception e) {
            throw Exceptions.throwAsRuntime(e);
        }
    }
...
//        return new SparkDataSet(rdd.map(new RowToLocatedRowFunction(context))); TODO
        return null;
    }
