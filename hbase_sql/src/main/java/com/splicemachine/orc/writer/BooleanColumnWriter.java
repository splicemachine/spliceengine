...
import com.splicemachine.orc.metadata.statistics.BooleanStatisticsBuilder;
import com.splicemachine.orc.metadata.statistics.ColumnStatistics;
import com.splicemachine.orc.stream.BooleanOutputStream;
        ...
        implements ColumnWriter
{
    ...
    public BooleanColumnWriter(int column, DataType type, CompressionKind compression, int bufferSize)
    {
    ...
    public void writeBlock(ColumnBlock block)
    {
        ...
        for (int position = 0; position < block.getPositionCount(); position++) {
            if (!block.isNull(position)) {
                boolean value = block.getBoolean(position);
