...
import com.splicemachine.orc.metadata.statistics.DoubleStatisticsBuilder;
import com.splicemachine.orc.stream.DoubleOutputStream;
        ...
        implements ColumnWriter
{
    ...
    public DoubleColumnWriter(int column, DataType type, CompressionKind compression, int bufferSize)
    {
    ...
    public void writeBlock(ColumnBlock block)
    {
        ...
        for (int position = 0; position < block.getPositionCount(); position++) {
            if (!block.isNull(position)) {
                double value = block.getDouble(position);
