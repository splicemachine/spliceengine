...
import com.splicemachine.orc.metadata.statistics.SliceColumnStatisticsBuilder;
import com.splicemachine.orc.stream.ByteArrayOutputStream;
        ...
        implements ColumnWriter
{
    ...
    public SliceDirectColumnWriter(int column, DataType type, CompressionKind compression, int bufferSize, boolean isDwrf, Supplier<SliceColumnStatisticsBuilder> statisticsBuilderSupplier)
    {
    ...
    public void writeBlock(ColumnBlock block)
    {
        ...
        for (int position = 0; position < block.getPositionCount(); position++) {
            if (!block.isNull(position)) {
                UTF8String string = block.getUTF8String(position);
                Slice value = Slices.utf8Slice(string.toString()); // TODO Make Faster JL
