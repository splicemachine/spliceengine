...
public class OrcRecordReader
{
    ...
    public ColumnarBatch getColumnarBatch(StructType schema) throws IOException {
        ...
        for (int i = 0; i < schema.fields().length; i++){
            int col = Integer.parseInt(schema.fieldNames()[i].substring(1));
            if (partitionIds.size() > 0 && col == partitionIds.get(0)) {
                columnBlocks[i] = new LazyColumnBlock(new LazyPartitionColumnBlockLoaderImpl(fields[i].dataType(), currentBatchSize, partitionValues.get(0))); // possible partitioned column
            } else {
                columnBlocks[i] = new LazyColumnBlock(new LazyIncludedColumnBlockLoaderImpl(streamReaders[col],fields[i].dataType())); // other columns
            }
        }
        ...
        for (ColumnBlock columnBlock: columnBlocks) {
            if (columnBlock != null) { // added while debugging SPLICE-1612.
            ...
            Map<Integer, DataType> includedColumns)
    {
        ...
        StreamReader[] streamReaders = new StreamReader[rowType.getFieldCount()+1];

        // store the adjusted column locations. SPLICE-1621
        int[] actualCols = new int[streamDescriptors.size()+1];
