...
@SuppressFBWarnings(value = Array("ST_WRITE_TO_STATIC_FROM_INSTANCE_METHOD", "NP_ALWAYS_NULL", "EI_EXPOSE_REP2"), justification = "Need to set SpliceClient.connectionString|These fields usually are not null|The prunedFields value is needed and is used read-only")
