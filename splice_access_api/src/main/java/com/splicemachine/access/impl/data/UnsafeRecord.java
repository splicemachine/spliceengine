...
public class UnsafeRecord implements Record<byte[]> {
    ...
    public int[] bitSetToIntArray(BitSet accessedColumns, DataValueDescriptor[] dvds) {
        if (accessedColumns == null) {
            ...
            int[] columns = new int[(int)accessedColumns.cardinality()];
            int i = 0;
            int j = 0;
            while ( (i = accessedColumns.nextSetBit(i)) !=-1) {
                columns[j] = i;
                i++; // This bitset is inclusive of the range, must increment.
                j++;
            }
            return columns;
        }
    ...
    public void getData(BitSet accessedColumns, DataValueDescriptor[] dvds) throws StandardException {
            getData(bitSetToIntArray(accessedColumns,dvds),dvds);
    }

    @Override
    public void getData(BitSet accessedColumns, DataValueDescriptor[] rows, int[] mainColToIndexPosMap) throws StandardException {
            getData(bitSetToIntArray(accessedColumns,rows),rows,mainColToIndexPosMap);
    }
        ...
        for (int i = 0; i< columns.length; i++) {
            do {
                if (fromIndex == -1 || fromIndex > columns[i]) {// Exhausted or past record
                    if (rows[i] != null) // sparse arrays from the dictionary...
                        rows[i].setToNull();
                    break;
                }
    ...
    @Override
    public void mergeWithExistingData(BitSet accessedColumns, DataValueDescriptor[] rows, int[] indexToMainColumnMap) throws StandardException {
        mergeWithExistingData(bitSetToIntArray(accessedColumns,rows),rows,indexToMainColumnMap);
    }
    ...
    public void mergeWithExistingData(int[] columns, DataValueDescriptor[] rows, int[] indexToMainColumnMap) throws StandardException {
        ...
        for (int i = 0; i< columns.length; i++) {
            do {
                if (fromIndex == -1 || fromIndex > columns[i]) {// Exhausted or past record
//                    if (rows[indexToMainColumnMap[columns[i]]] != null) // sparse arrays from the dictionary...
//                        rows[indexToMainColumnMap[columns[i]]].setToNull();
                    break;
                }
                if (fromIndex == columns[i]) { // set value and move on
    ...
    public void getData(int[] columns, ExecRow execRow) throws StandardException {
        getData(columns,execRow.getRowArray());
    }
    ...
    @Override
    public Record applyRedo(Iterator<Record<byte[]>> recordIterator, FormatableBitSet variableLength) throws StandardException {
        Record rolledBackRecord = this;
        while (recordIterator.hasNext()) {
            rolledBackRecord = rolledBackRecord.applyRedo(recordIterator.next(),variableLength);
        }
        return rolledBackRecord;
    }

    @Override
    public Record applyRedo(Record<byte[]> updatedRecord, ExecRow recordDefinition) throws StandardException {
        return applyRedo(updatedRecord,recordDefinition.getVariableLengthBitSet());
    }

    @Override
    public Record applyRedo(Record<byte[]> updatedRecord, FormatableBitSet variableLength, FormatableBitSet accessedColumns) throws StandardException {
        ...
        try {
            while (fromIndex != -1) {
                if (accessedColumns == null || (accessedColumns.getLength() > fromIndex && accessedColumns.isSet(fromIndex))) {
                    if (UnsafeRecordUtils.isSet(uR.baseObject, uR.baseOffset + COLS_BS_INC, fromIndex)) { // Has Update Entry
                        if (UnsafeRecordUtils.isSet(newActiveBitSetArray, ASIZE, fromIndex)) { // Required for Active write and we know nulls are already removed
                            copyUnsafeData(variableLength.getLength() > accessed && variableLength.isSet(accessed), updateRow, updatePos, newActiveBuffer, newActiveRow, newActivePos);
                            newActivePos++;
                        ...
                        copyUnsafeData(variableLength.getLength() > accessed && variableLength.isSet(accessed), activeRow, activePos, newActiveBuffer, newActiveRow, newActivePos);
                        activePos++;
                        newActivePos++;
                    }
                    accessed++;
    ...
    @Override
    public Record applyRedo(Record<byte[]> updatedRecord, FormatableBitSet variableLength) throws StandardException {
        return applyRedo(updatedRecord,variableLength,null);
    }

    @Override
    public Record[] updateRecord(Record updatedRecord, FormatableBitSet variableLength) throws StandardException {
        if (updatedRecord.hasTombstone()) { // delete record
            updatedRecord.setVersion(getVersion()+1);
            // Must Remove Record
        ...
        while (fromIndex != -1) {
            if (UnsafeRecordUtils.isSet(uR.baseObject,uR.baseOffset+COLS_BS_INC,fromIndex)) { // Has Update Entry
                if (UnsafeRecordUtils.isSet(newActiveBitSetArray,ASIZE,fromIndex)) { // Required for Active write and we know nulls are already removed
                    copyUnsafeData(variableLength.isSet(fromIndex), updateRow, updatePos, newActiveBuffer, newActiveRow, newActivePos);
                    newActivePos++;
                    if (UnsafeRecordUtils.isSet(baseObject,baseOffset+COLS_BS_INC,fromIndex)) { // Has Active Entry
                        // write active to newRedo
                        copyUnsafeData(variableLength.isSet(fromIndex), activeRow, activePos, newRedoBuffer, newRedoRow, newRedoPos);
                        activePos++;
                    } else {
                        // Write Null to redo
                ...
                // write active to newActive
                copyUnsafeData(variableLength.isSet(fromIndex), activeRow, activePos, newActiveBuffer, newActiveRow, newActivePos);
                activePos++;
                newActivePos++;
            }
            fromIndex = UnsafeRecordUtils.nextSetBit(globalOrBitSetArray,ASIZE,fromIndex+1,bitSetWords);
     ...
     * @throws StandardException
     */
    public void copyUnsafeData(boolean variableLength, UnsafeRow dataRow, int dataOrdinal, BufferHolder newBuffer, UnsafeRow newDataRow, int newDataOrdinal) throws StandardException{
        if (variableLength) {
