...
public class UnsafeRecord implements Record<byte[]> {
    ...
    public static int TOMB_INC = TOTAL_BYTES;//+8;
    ...
    public UnsafeRecord(DataCell dataCell) {
        ...
        this.baseOffset = dataCell.valueOffset()+16;
    ...
    public KVPair getKVPair() {
        if (hasTombstone())
            return new KVPair(keyObject,(int)keyOffset,(int)keyLength,(byte[]) baseObject,0,EFF_TS_INC, KVPair.Type.DELETE);
    ...
    public void setHasTombstone(boolean hasTombstone) {
        ...
        baseLength = EFF_TS_INC; // Trim the data for the key value store when generated...  Just need tombstone and txn info...
    }
    ...
    public byte[] getValue() {
        ...
        Platform.copyMemory(baseObject,baseOffset,value,16,baseLength);
        return value;
    }
    ...
    private UnsafeRow getUnsafeRow() {
        int numberOfColumns = numberOfColumns();
        int bitSetWidth = UnsafeRecordUtils.calculateBitSetWidthInBytes(numberOfColumns);
        UnsafeRow activeRow = new UnsafeRow(UnsafeRecordUtils.cardinality(baseObject,baseOffset+21,bitSetWidth/8));
        activeRow.pointTo(baseObject,baseOffset+UNSAFE_INC+bitSetWidth,Platform.getInt(baseObject,baseOffset+21+bitSetWidth+4));
        return activeRow;
    }
