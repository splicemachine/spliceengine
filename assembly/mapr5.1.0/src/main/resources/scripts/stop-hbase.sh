...
# Stop HBase services on MapR cluster nodes.  Needs to be run on control node.
...
show_help()
{
    echo "Stop HBase services on MapR cluster nodes.  Needs to be run on control node."
...
# Stop all HBase RegionServers and then Masters
echo "STOPPING all HBase RegionServers"
sudo maprcli node services -hbregionserver stop -filter "${mapr_hnfilter}and[csvc==hbregionserver]"
echo "STOPPING all HBase Masters"
sudo maprcli node services -hbmaster stop -filter "${mapr_hnfilter}and[csvc==hbmaster]"
