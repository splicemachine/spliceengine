...
public class SimpleStore implements RelationReader, RelationWriter {
    private final Map<String, Map<String, SimpleLock>> locks = new HashMap<String, Map<String, SimpleLock>>();
    private final Map<String, Map<SimpleLock, String>> reverseLocks = new HashMap<String, Map<SimpleLock, String>>();
    ...
    public RowLock lockRow(Relation relation, Object row) {
        synchronized (this) {
            String table = ((SimpleRelation) relation).relationIdentifier;
            Map<String, SimpleLock> lockTable = locks.get(table);
            Map<SimpleLock, String> reverseLockTable = reverseLocks.get(table);
            if (lockTable == null) {
                lockTable = new HashMap<String, SimpleLock>();
                reverseLockTable = new HashMap<SimpleLock, String>();
                locks.put(table, lockTable);
                reverseLocks.put(table, reverseLockTable);
            }
            SimpleLock currentLock = lockTable.get(row);
            if (currentLock == null) {
