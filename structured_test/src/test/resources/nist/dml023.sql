...
-- TEST:0106 Comparison predicate <> !
     SELECT PNUM                                                         
          FROM PROJ                                                          
          WHERE CITY <> 'Deale' order by pnum;
...
-- splicetest: ignore-order start
     SELECT empnum
          FROM WORKS
          WHERE EMPNUM = 'E1' order by empnum;
-- splicetest: ignore-order stop
-- PASS:0107 If count = 6 ?

-- The result is implementation dependent and will not be fixed for Splice
--O     SELECT COUNT(*)
--     SELECT empnum
--          FROM WORKS
