...
-- Added order by
     SELECT COUNT(*)
           FROM STAFF ORDER BY EMPNUM,EMPNAME,GRADE,CITY;
...
--Bug 376     DELETE FROM WORKS
--Bug 376           WHERE WORKS.PNUM IN
--Bug 376                 (SELECT PROJ.PNUM
--Bug 376                       FROM PROJ
--Bug 376                       WHERE PROJ.PNUM=WORKS.PNUM
--Bug 376                       AND PROJ.CITY='Tampa');
...
-- Added order by
    SELECT COUNT(*)
           FROM WORKS ORDER BY EMPNUM,PNUM,HOURS;
-- PASS:0038 If count = 12?
