...
--O  AND TABLE_TYPE = 'BASE TABLE';
     select count(*) from SYS.SYSTABLES a, SYS.SYSCONGLOMERATES b ,sys.sysschemas c where a.TABLEID = b.TABLEID and tablename in ('LOTSA_TYPES') and b.schemaid = c.schemaid and SCHEMANAME in ('FLATER');
--O-- PASS:0679 If count = 1?
--O
--O   SELECT COUNT(*) FROM INFO_SCHEM.VIEWS
--O  WHERE TABLE_SCHEM = 'FLATER' AND TABLE_NAME = 'LOTSA_TYPES';
     select count(*) from SYS.SYSTABLES a, SYS.SYSVIEWS b ,sys.sysschemas c where a.TABLEID = b.TABLEID and tablename in ('LOTSA_TYPES') and a.schemaid = c.schemaid and SCHEMANAME in ('FLATER');
...
--O  NULL AND IS_NULLABLE = 'YES';
     select count(*) from SYS.SYSTABLES a, SYS.SYSCOLUMNS b ,sys.sysschemas c where tablename in ('LOTSA_TYPES') and a.schemaid = c.schemaid and SCHEMANAME in ('FLATER')
 AND COLUMNNAME = 'FLAAG' AND COLUMNNUMBER = 1;
--O-- PASS:0679 If count = 1?
--O
--O   SELECT COUNT(*) FROM INFO_SCHEM.COLUMNS
--O  WHERE TABLE_SCHEM = 'FLATER' AND TABLE_NAME = 'LOTSA_TYPES'
