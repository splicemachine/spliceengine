...
-- added order by
      SELECT * FROM HU.PROJ ORDER BY PNUM,PNAME,PTYPE,BUDGET,CITY;
-- PASS:Setup if count = 6?

--O      SELECT COUNT(*) FROM HU.STAFF;
-- added order by
      SELECT * FROM HU.STAFF ORDER BY EMPNUM,EMPNAME,GRADE,CITY;
-- PASS:Setup if count = 5?

--O      SELECT COUNT(*) FROM HU.WORKS;
-- added order by
      SELECT * FROM HU.WORKS ORDER BY EMPNUM,PNUM,HOURS;
...
--O      SELECT COUNT(*) FROM HU.STAFF3;
      SELECT * FROM HU.STAFF3 ORDER BY EMPNUM,EMPNAME,GRADE,CITY;
-- PASS:Setup if count = 5?
