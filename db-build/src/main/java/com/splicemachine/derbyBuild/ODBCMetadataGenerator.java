/*
   ...
   Derby - Class com.splicemachine.db.catalog.ODBCProcedureColsVTI
 ...
 * this class reads from the metadata.properties
...
public class ODBCMetadataGenerator {
    ...
    private final String ODBC_QUERY_NOTICE =
            "#\n# *** NOTE! ***  The following query was generated\n" +
                    ...
                    "# class in the com/splicemachine/derbyBuild directory.\n";
     ...
     */
    public ODBCMetadataGenerator(File outputFile) throws IOException {

        // SQL fragments.
        odbcFragments = new Properties();
        odbcFragments.load(this.getClass().getResourceAsStream("odbcgen_fragments.properties"));

        // Prep output file.
        odbcMetaFile = new FileWriter(outputFile);
    }
    ...
    public static void main(String[] arguments) throws IOException {
        ...
        if(arguments == null || arguments.length != 2) {
            throw new IllegalArgumentException("expected two args: <input property file> <output property file>");
        }

        String sourceMetadataPropertiesFilename = arguments[0];
        File sourceMetadataPropertiesFile = new File(sourceMetadataPropertiesFilename);
        if(!sourceMetadataPropertiesFile.exists()) {
            throw new IllegalArgumentException("file does not exist: " +  sourceMetadataPropertiesFilename);
        }

        String targetPropertiesFilename = arguments[1];
        File targetPropertiesFile = new File(targetPropertiesFilename);
        targetPropertiesFile.getParentFile().mkdirs();

        ODBCMetadataGenerator odbcGen = new ODBCMetadataGenerator(targetPropertiesFile);
        odbcGen.initChanges();
        InputStream resourceAsStream = new FileInputStream(sourceMetadataPropertiesFile);
        odbcGen.generateODBCQueries(resourceAsStream);
    ...
    private void initChanges() {
        ...
        changeMap.put("getProcedureColumns", (byte) (COL_RENAME_CHANGE | TYPE_VALUE_CHANGE | ADD_COLUMN_CHANGE));
        ...
        changeMap.put("getTypeInfo", (byte) (COL_RENAME_CHANGE | TYPE_VALUE_CHANGE | ADD_COLUMN_CHANGE));
     ...
     */
    public void generateODBCQueries(InputStream is) throws IOException {

        // JDBC query that we read from metadata.properties.
        StringBuilder query = new StringBuilder();

        // Note: We use ISO-8859-1 because property files are defined to be that encoding.
        LineNumberReader reader = new LineNumberReader(new InputStreamReader(is, "ISO-8859-1"));
    ...
    private void renameColForODBC(StringBuilder queryText,
        ...
        while (pos != -1) {
            ...
            if (((pos2 - 2) > 0) && (queryString.charAt(pos2 - 2) == 'A')
                    && (queryString.charAt(pos2 - 1) == 'S')) { // then this is the one we want to replace.
                break;
            } else {
    ...
    private void generateSELECTClause(String queryName,
        ...
        if (!stmtNeedsChange(queryName, TYPE_VALUE_CHANGE) &&
                !stmtNeedsChange(queryName, ADD_COLUMN_CHANGE)) { // then we don't need to generate a SELECT, because we
        ...
        for (int i = 0; i < selectColDefs.size(); i++) {
            ...
            // sql standard.
            //
            if (isBoolean(colName)) {
                newQueryText.append(" ( CASE WHEN ");
            }
            ...
            if (isBoolean(colName)) {
                newQueryText.append(" THEN 1 ELSE 0 END ) ");
            }
            if (castInfo != null) {
        ...
        for (int i = start; i < queryChars.length; i++) {
            ...
            if (queryChars[i] == '(')
                ...
                start = i + 1;
                    ...
                    && (queryChars[i + 1] == 'R')
                    && (queryChars[i + 2] == 'O')
                    && (queryChars[i + 3] == 'M')) { // this is the end of final column definition; store it
    ...
    private void substitutePatternWhere(String oldPattern, String newPattern,
        ...
        //Rather, verify that
        //classes/org/apache/db/impl/jdbc/metadata.properties is
    ...
    private boolean isBoolean(String colName) {
        for (String BOOLEAN_COLUMN : BOOLEAN_COLUMNS) {
            if (BOOLEAN_COLUMN.equals(colName)) {
                return true;
            }
        }
        return false;
    ...
    private boolean stmtNeedsChange(String queryName, byte changeType) {
        ...
        return changeByte != null && ((changeByte & changeType) == changeType);
     ...
     *
     * @param fragId id of the fragment to look up.
