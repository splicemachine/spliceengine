...
{
	private static final Logger LOG = org.apache.logging.log4j.LogManager.getLogger(ContextManager.class);
    ...
    public boolean cleanupOnError(Throwable error, boolean diagActive)
	{
		...
		else if (error instanceof PassThroughException) {
				error = error.getCause();
		}
		else if (!(error instanceof StandardException)) {
