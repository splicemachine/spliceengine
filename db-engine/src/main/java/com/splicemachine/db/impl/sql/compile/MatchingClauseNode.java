...
public class MatchingClauseNode extends QueryTreeNode {
    ...
    void bind(DataDictionary dd, MergeNode mergeNode, FromList fullFromList, FromBaseTable targetTable)
            throws StandardException {
        //
        ...
        if (isDeleteClause()) {
            throw new RuntimeException("DELETE operation not yet supported");
//            bindDelete( dd, fullFromList, targetTable );
        }
        if (isUpdateClause()) {
            bindUpdate(dd, mergeNode, fullFromList, targetTable);
        }
        if (isInsertClause()) {
    ...
    private ResultColumnList realiasSetClauses( FromBaseTable targetTable ) throws StandardException
    {
        ...
        for ( int i = 0; i < _updateColumns.size(); i++ )
        {
            ...
            ColumnReference newTargetColumn = new ColumnReference( setRC.getReference().getColumnName(),
                            tableName, getContextManager() );
            newTargetColumn.setMergeTableID( ColumnReference.MERGE_TARGET );
            ResultColumn newRC = new ResultColumn( newTargetColumn, setRC.getExpression(), getContextManager() );
    ...
    private void bindSetClauses( MergeNode mergeNode, FromList fullFromList, FromTable targetTable,
     ...
     * </p>
     */
    private void buildThenColumnsForUpdate(FromList fullFromList, FromTable targetTable,
                                           ResultColumnList fullRow, ResultColumnList beforeRow,
     ...
     * </ul>
     * @param changedColumnNames columns which are explicitly mentioned on the left side of a SET clause
     */
    private HashSet<String> getChangedGeneratedColumnNames(TableDescriptor targetTableDescriptor,
                                                           HashSet<String> changedColumnNames
    )
            throws StandardException {
        HashSet<String> result = new HashSet<String>();
...
//        );
     ...
     * </p>
     */
    void generate(ActivationClassBuilder acb, ResultColumnList selectList, ResultSetNode  generatedScan,
            HalfOuterJoinNode hojn, int clauseNumber)
            throws StandardException
    {
     ...
     * </p>
     */
    private void generateInsertUpdateRow( ActivationClassBuilder acb, ResultColumnList selectList,
                                          ResultSetNode generatedScan, HalfOuterJoinNode hojn)
            throws StandardException
    {
     ...
     * </p>
     */
    private void adjustMatchingRefinement( ResultColumnList selectList, ResultSetNode generatedScan )
            throws StandardException
    {
        if (_matchingRefinement != null) {
     ...
     * </p>
     */
    private void adjustThenColumns( ResultColumnList selectList, ResultSetNode generatedScan,
            HalfOuterJoinNode hojn ) throws StandardException
    {
     ...
     * </p>
     */
    private void useGeneratedScan( ResultColumnList selectList,ResultSetNode generatedScan, QueryTreeNode node)
            throws StandardException
    {
        ResultColumnList leftJoinResult = generatedScan.getResultColumns();
    ...
    private List<ColumnReference> getColumnReferences(QueryTreeNode expression )
        throws StandardException
    {
        CollectNodesVisitorT<ColumnReference> getCRs =
                new CollectNodesVisitorT<ColumnReference>(ColumnReference.class);
