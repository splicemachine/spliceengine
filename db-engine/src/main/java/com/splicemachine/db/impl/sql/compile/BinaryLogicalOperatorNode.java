...
abstract class BinaryLogicalOperatorNode extends BinaryOperatorNode
{
    ...
    public ValueNode bindExpression(FromList fromList,
        ...
        if (getLeftOperand().isParameterNode() || getRightOperand().isParameterNode())
    ...
    boolean verifyEliminateNots() {
        if (SanityManager.ASSERT)
         {
             return (getLeftOperand().verifyEliminateNots() &&
                                getRightOperand().verifyEliminateNots());
        } else {return true;}
     }
    ...
    public double getBaseOperationCost() throws StandardException {
        return SIMPLE_OP_COST + super.getBaseOperationCost();
    }
