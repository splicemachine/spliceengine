...
public class DataDictionaryCache {
    ...
    public DataDictionaryCache(Properties startParams,DataDictionary dd) throws StandardException {
        ...
        oidTdCache = new ManagedCache<>(CacheBuilder.newBuilder().maximumSize(tdCacheSize).build());
        nameTdCache = new ManagedCache<>(CacheBuilder.newBuilder().maximumSize(tdCacheSize).build());
        if(stmtCacheSize>0){
            spsNameCache = new ManagedCache<>(CacheBuilder.newBuilder().maximumSize(stmtCacheSize).removalListener(dependentInvalidator).build());
            storedPreparedStatementCache = new ManagedCache<>(CacheBuilder.newBuilder().maximumSize(stmtCacheSize).removalListener(dependentInvalidator).build());
        }
        sequenceGeneratorCache=new ManagedCache<>(CacheBuilder.newBuilder().maximumSize(seqgenCacheSize).build());
        partitionStatisticsCache = new ManagedCache<>(CacheBuilder.newBuilder().maximumSize(8092).build());
        conglomerateCache = new ManagedCache<>(CacheBuilder.newBuilder().maximumSize(1024).build());
        statementCache = new ManagedCache<>(CacheBuilder.newBuilder().maximumSize(1024).removalListener(dependentInvalidator).build());
        schemaCache = new ManagedCache<>(CacheBuilder.newBuilder().maximumSize(1024).build());
        aliasDescriptorCache = new ManagedCache<>(CacheBuilder.newBuilder().maximumSize(1024).build());
        roleCache = new ManagedCache<>(CacheBuilder.newBuilder().maximumSize(100).build());
        permissionsCache=new ManagedCache<>(CacheBuilder.newBuilder().maximumSize(permissionsCacheSize).build());
        this.dd = dd;
    }
