...
public class DataDictionaryCache {
    ...
    public DataDictionaryCache(Properties startParams,DataDictionary dd) throws StandardException {
        ...
        if(spsCacheSize>0){
            spsNameCache = new ManagedCache<>(CacheBuilder.newBuilder().recordStats().maximumSize(spsCacheSize).build(), spsCacheSize);
            storedPreparedStatementCache = new ManagedCache<>(CacheBuilder.newBuilder().recordStats().maximumSize(spsCacheSize).build(), spsCacheSize);
        }
