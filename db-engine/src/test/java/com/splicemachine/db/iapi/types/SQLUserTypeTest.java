 ...
 *
 */
public class SQLUserTypeTest extends SQLDataValueDescriptorTest {

        @Test
        public void serdeValueData() throws Exception {
                UnsafeRow row = new UnsafeRow(1);
                UnsafeRowWriter writer = new UnsafeRowWriter(new BufferHolder(row),1);
                UserType value = new UserType(new SQLChar("Splice Machine"));
                UserType valueA = new UserType();
                writer.reset();
                value.write(writer, 0);
                Assert.assertEquals("SerdeIncorrect","Splice Machine", ((SQLChar) SerializationUtils.deserialize(row.getBinary(0))).toString());
                valueA.read(row,0);
