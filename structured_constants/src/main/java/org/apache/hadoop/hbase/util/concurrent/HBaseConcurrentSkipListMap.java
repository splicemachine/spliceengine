package org.apache.hadoop.hbase.util.concurrent;
...
import java.util.concurrent.ConcurrentNavigableMap;
	 ...
	 * @since 1.6
	 */
	public class HBaseConcurrentSkipListMap<K,V> extends AbstractMap<K,V>
	     ...
	     */
	    public HBaseConcurrentSkipListMap() {
	     ...
	     */
	    public HBaseConcurrentSkipListMap(Comparator<? super K> comparator) {
	     ...
	     *         or values are null
	     */
	    public HBaseConcurrentSkipListMap(Map<? extends K, ? extends V> m) {
	        this.comparator = null;
	        initialize();
	        putAll(m);
	    }
	     ...
	     */
	    public HBaseConcurrentSkipListMap(SortedMap<K, ? extends V> m) {
	     ...
	     */
	    public HBaseConcurrentSkipListMap<K,V> clone() {
	    	HBaseConcurrentSkipListMap<K,V> clone = null;
	        try {
	            clone = (HBaseConcurrentSkipListMap<K,V>) super.clone();
	    ...
	    abstract class Iter<T> implements Iterator<T> {
	        ...
	        public void remove() {
	            ...
	            HBaseConcurrentSkipListMap.this.remove(l.key);
	            lastReturned = null;
	        }
	    ...
	    static final class KeySet<E>
	        ...
	        public Iterator<E> iterator() {
	            if (m instanceof HBaseConcurrentSkipListMap)
	                return ((HBaseConcurrentSkipListMap<E,Object>)m).keyIterator();
	            else
	                return ((HBaseConcurrentSkipListMap.SubMap<E,Object>)m).keyIterator();
	        }
	        public boolean equals(Object o) {
	            if (o == this)
	                return true;
	    ...
	    static final class Values<E> extends AbstractCollection<E> {
	        ...
	        public Iterator<E> iterator() {
	            if (m instanceof HBaseConcurrentSkipListMap)
	                return ((HBaseConcurrentSkipListMap<Object,E>)m).valueIterator();
	            else
	    ...
	    static final class EntrySet<K1,V1> extends AbstractSet<Map.Entry<K1,V1>> {
	        ...
	        public Iterator<Map.Entry<K1,V1>> iterator() {
	            if (m instanceof HBaseConcurrentSkipListMap)
	                return ((HBaseConcurrentSkipListMap<K1,V1>)m).entryIterator();
	            else
	    ...
	    static final class SubMap<K,V> extends AbstractMap<K,V>
	        ...
	        /** Underlying map */
	        private final HBaseConcurrentSkipListMap<K,V> m;
	         ...
	         */
	        SubMap(HBaseConcurrentSkipListMap<K,V> map,
	         ...
	         */
	        private boolean isBeforeEnd(HBaseConcurrentSkipListMap.Node<K,V> n) {
	            if (n == null)
	                return false;
	            if (hi == null)
	                return true;
	         ...
	         */
	        private HBaseConcurrentSkipListMap.Node<K,V> loNode() {
	            if (lo == null)
	                return m.findFirst();
	            else if (loInclusive)
	         ...
	         * most usages need to check bounds
	         */
	        private HBaseConcurrentSkipListMap.Node<K,V> hiNode() {
	            if (hi == null)
	                return m.findLast();
	            else if (hiInclusive)
	         ...
	         */
	        private K lowestKey() {
	        	HBaseConcurrentSkipListMap.Node<K,V> n = loNode();
	            if (isBeforeEnd(n))
	                return n.key;
	            else
	                throw new NoSuchElementException();
	        ...
	        private K highestKey() {
	        	HBaseConcurrentSkipListMap.Node<K,V> n = hiNode();
	            if (n != null) {
	                K last = n.key;
	                if (inBounds(last))
	                    return last;
	        ...
	        public int size() {
	            long count = 0;
	            for (HBaseConcurrentSkipListMap.Node<K,V> n = loNode();
	                 isBeforeEnd(n);
	                 n = n.next) {
	                if (n.getValidValue() != null)
	                    ++count;
	    ...
	    static {
	        try {
	            UNSAFE = sun.misc.Unsafe.getUnsafe();
	            Class k = HBaseConcurrentSkipListMap.class;
