...
public class ReadAheadRegionScanner implements MeasuredRegionScanner{
		...
		private final RingBuffer<List<Cell>> ringBuffer;
		...
		protected boolean getFromBatch(List<Cell> result, long nextSequence) {
				if(nextSequence<=availableSequence){
						/*
						 * We are still within the same batch, can read one directly
						 */
						try{
								List<Cell> kvs = ringBuffer.get(nextSequence);
								if(kvs.size()<=0) {
										completed = true;
										return false;
								}
		...
		@Override public boolean next(List<Cell> results) throws IOException { return nextRaw(results); }
        ...
        @Override
        public long getMaxResultSize() { return delegate.getMaxResultSize(); }

		/*private helper methods*/
		/********************************************************************************************************************/

		private static final EventTranslatorOneArg<List<Cell>,List<Cell>> translator =
						new EventTranslatorOneArg<List<Cell>,List<Cell>>() {
								@Override
								public void translateTo(List<Cell> event, long sequence, List<Cell> arg0) {
										event.clear();
										event.addAll(arg0);
								}
						};
		...
		private class Reader implements Callable<IOStats> {
				...
				public IOStats call() throws Exception {
						...
						List<Cell> readKvs = Lists.newArrayListWithExpectedSize(2);
						...
						OUTER: do{
								...
								try{
										...
										for(int batch=0;batch<LOCK_SIZE && shouldContinue;batch++){
												readKvs.clear();
												shouldContinue = delegate.nextRaw(readKvs);
												if(readKvs.size()<=0) break OUTER;

												rowCount++;
												if(bytesCounter.isActive()){
														for(Cell kv:readKvs){
																bytesCounter.add(kv.getRowLength());
														}
												}
		...
		private static class ListFactory implements EventFactory<List<Cell>> {
				@Override public List<Cell> newInstance() {
