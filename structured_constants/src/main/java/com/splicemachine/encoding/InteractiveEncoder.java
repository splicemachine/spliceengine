    ...
    private enum DataType{
        ...
        SORTED_BYTES("a"){
            ...
            public byte[] encode(String text) {
                ...
                return Encoding.encode(toBytesBinary(text));
            }

            @Override
            public String decode(byte[] bytes) {
                return toStringBinary(Encoding.decodeBytes(bytes),0,bytes.length);
            }
        },
        UNSORTED_BYTES("u"){
            @Override
            public byte[] encode(String text) {
                //two possible forms--HBase hex, or raw hex. Both can be treated using toBytesBinary
                return Encoding.encodeBytesUnsorted(toBytesBinary(text));
            }

            @Override
            public String decode(byte[] bytes) {
                byte[] decoded = Encoding.decodeBytesUnsortd(bytes,0,bytes.length);
                return toStringBinary(decoded,0,decoded.length);
            }
        } ;
