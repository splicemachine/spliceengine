...
public class JoinOperationIT {
    ...
    public void testInnerJoinNoRestriction() throws Exception {
        ResultSet rs = methodWatcher.executeQuery(String.format(
                "select count(*), max(foo.col1) from foo, foo2 --Splice-properties joinStrategy=%s\n" +
                "where foo.col1 = foo.col2"
        ));
        rs.next();
        Assert.assertEquals(String.format("Missing Records for %s", joinStrategy),3,rs.getInt(1));
        Assert.assertEquals(String.format("Wrong max for %s", joinStrategy),3,rs.getInt(2));
    }

    @Test
    public void testInnerJoinRestriction() throws Exception {
        ResultSet rs = methodWatcher.executeQuery(String.format(
                "select count(*), min(foo.col1) from foo inner join foo2 --Splice-properties joinStrategy=%s\n" +
                        "on foo.col1 = foo.col2 and foo.col1+foo.col2>6"
        ));
        rs.next();
    ...
    public void testInnerAntiJoinNoRestriction() throws Exception {
        ResultSet rs = methodWatcher.executeQuery(String.format(
                "select count(*) from foo where not exists (select * from foo2 --Splice-properties joinStrategy=%s\n" +
                        "where foo.col1 = foo.col2)"
        ));
        rs.next();
        Assert.assertEquals(String.format("Missing Records for %s", joinStrategy),2,rs.getInt(1));
    ...
    public void testInnerAntiJoinRestriction() throws Exception {
        ResultSet rs = methodWatcher.executeQuery(String.format(
                "select count(*) from foo where not exists (select * from foo2 --Splice-properties joinStrategy=%s\n" +
                        "where foo.col1 = foo.col2 and foo.col1+foo.col2>6)"
        ));
        rs.next();
        Assert.assertEquals(String.format("Missing Records for %s", joinStrategy),3,rs.getInt(1));
    }
    ...
    public void testOuterJoinNoRestriction() throws Exception {
        ResultSet rs = methodWatcher.executeQuery(String.format(
                "select count(*), sum(CASE WHEN foo2.col1 is null THEN 0 ELSE 1 END) from foo, foo2 --Splice-properties joinStrategy=%s\n" +
                        "where foo.col1 = foo.col2"
        ));
        rs.next();
    ...
    public void testOuterJoinRestriction() throws Exception {
        ResultSet rs = methodWatcher.executeQuery(String.format(
                "select count(*), sum(CASE WHEN foo2.col1 is null THEN 0 ELSE 1 END) from foo, foo inner join foo2 --Splice-properties joinStrategy=%s\n" +
                        "on foo.col1 = foo.col2 and foo.col1+foo.col2>6"
        ));
        rs.next();
        Assert.assertEquals(String.format("Missing Records for %s", joinStrategy),5,rs.getInt(1));
