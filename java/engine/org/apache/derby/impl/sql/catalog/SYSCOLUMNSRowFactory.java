...
public class SYSCOLUMNSRowFactory extends CatalogRowFactory
{
            ...
            throws StandardException
    {
        ...
        /*10th column is a flag for whether or not statistics are collected for this column*/
        /*
    ...
    public TupleDescriptor buildDescriptor( ExecRow	row, TupleDescriptor parentTupleDescriptor, DataDictionary dd )
        ...
        /*
         * For backwards compatibility, we know that the COLLECT_STATS column doesn't
         * exist for some databases, so when we describe the data, we have to ensure that
         * we deal with that case by adding a proper default value.
         *
         * As of version 1.1, we do *not* collect statistics by default, so we treat this as
         * false if we can't find it. Later versions may default collection to true for orderable
         * types and/or primary keys, but for now we'll keep it simple
         */
        DataValueDescriptor collectStatsColumn = row.getColumn(SYSCOLUMNS_COLLECTSTATS);
        boolean collectStats = false;
        if(collectStatsColumn!=null)
