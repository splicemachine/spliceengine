/*
   ...
   Derby - Class com.splicemachine.db.impl.store.raw.log.LogToFile
...
import com.splicemachine.db.iapi.services.diag.Performance;
...
import com.splicemachine.db.iapi.store.raw.log.Logger;
								  ...
								  Serviceable, java.security.PrivilegedExceptionAction
{
	...
	*/
    ...
    /**
     * When the db.system.durability property is set to 'test', the store
     ...
     * @see com.splicemachine.db.iapi.reference.Property#DURABILITY_PROPERTY
    ...
    // disable syncing of log file when running in db.system.durability=test
					...
					if (SanityManager.DEBUG_ON("setCheckpoint"))
					{
                        ...
                        else
                        {
                            SanityManager.THROWASSERT(
                                "must set db.storage.checkpointStartLog and db.storage.checkpointStartOffset, if setting setCheckpoint.");
                        }
	...
	*/
		 ...
		 throws IOException, StandardException
	{
        ...
        // When database is booted with db.system.durability=test,
		...
		try
		{
			...
			if ( onDiskChecksum == checksum.getValue() || onDiskChecksum ==0)
			{
                ...
                if (SanityManager.DEBUG) {
                    if (SanityManager.DEBUG_ON(LogToFile.DBG_FLAG))
                        SanityManager.DEBUG(LogToFile.DBG_FLAG,
                        "log control file, was db.system.durability set to test = " +
                        wasDBInDurabilityTestModeNoSync);
                }
	...
	*/
	public void	boot(boolean create, Properties startParams) throws StandardException
	{
		...
		logBufferSize =  PropertyUtil.getSystemInt(Property.LOG_BUFFER_SIZE,
												   LOG_BUFFER_SIZE_MIN, 
												   LOG_BUFFER_SIZE_MAX, 
		...
		//get log factorty properties if any set in db.properties
		getLogFactoryProperties(null);

		/* check if the storage factory supports write sync (rws and rwd). If
		 * so, use it unless db.storage.fileSyncTransactionLog property is
		 * set true by user.
		 */
		...
		if (logStorageFactory.supportsWriteSync())
        {
			...
			//disable write sync if db.storage.fileSyncTransactionLog is true
			isWriteSynced =
        ...
        // If db.system.durability=test is set,then set flag to
		...
		try
		{
			...
			if (!createNewLog)
			{
                if (privExists(logControlFileName))
				{
                    ...
                    // db.system.durability=test then print a message
                    // to the db log
		...
		// set max possible log file number to db 10.0 limit, if the database is not
	...
	*/
		 ...
		 throws StandardException
	{
		...
		if(set == null)
		{
			lsInterval=PropertyUtil.getSystemProperty(Property.LOG_SWITCH_INTERVAL);
			cpInterval=PropertyUtil.getSystemProperty(Property.CHECKPOINT_INTERVAL);
		}else
		{
			lsInterval = PropertyUtil.getServiceProperty(set, Property.LOG_SWITCH_INTERVAL);
			cpInterval = PropertyUtil.getServiceProperty(set, Property.CHECKPOINT_INTERVAL);
		}
	...
	*/
     ...
     * with db.system.durability=test or was previously at any time booted in
     * this mode, mention in the error message that the error is probably 
     * because the db.system.durability was set.
	...
	*/
     ...
     * @see com.splicemachine.db.impl.db.SlaveDatabase
     */
