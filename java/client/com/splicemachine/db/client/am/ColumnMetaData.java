/*
   ...
   Derby - Class com.splicemachine.db.client.am.ColumnMetaData
...
public class ColumnMetaData implements java.sql.ResultSetMetaData {
    ...
    // These are the db sql types, for use only by ResultSetMetaData, other code should use jdbcTypes_.
...
//    @Override
    ...
    public boolean hasLobColumns() {
        for (int i = 0; i < columns_; i++) {
            switch (com.splicemachine.db.client.am.Utils.getNonNullableSqlType(sqlType_[i])) {
            case DRDAConstants.DB2_SQLTYPE_BLOB:
            case DRDAConstants.DB2_SQLTYPE_CLOB:
                return true;
            default:
