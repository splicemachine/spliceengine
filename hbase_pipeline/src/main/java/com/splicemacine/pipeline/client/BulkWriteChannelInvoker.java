...
public class BulkWriteChannelInvoker {
    ...
    public BulkWritesResult invoke(BulkWrites write) throws IOException {
        ...
        Connection connection=HBaseConnectionFactory.getInstance().getConnection();
        NoRetryCoprocessorRpcChannel channel = new NoRetryCoprocessorRpcChannel(connection,tableName, write.getRegionKey());
    ...
    private boolean clearCacheIfNeeded(Throwable e) throws IOException{
        if (e instanceof WrongPartitionException ||
             ...
             */
            ((HConnection)HBaseConnectionFactory.getInstance().getConnection()).clearRegionCache(HBaseTableInfoFactory.getInstance().getTableInfo(tableName));
            return true;
	    }
        return false;
    }
