 ...
 *
 * <ol>
 *     <li>4-bit: high(counter)</li>
 *     <li>7-bit: high(machineId)</li>
 *     <li>41-bit: timestamp</li>
 *     <li>7-bit: low(counter)</li>
 *     <li>5-bit: low(machineId)</li>
 * </ol>
 *
 * @author Scott Fines
 * Created on: 6/21/13
...
public class Snowflake {
    ...
    private static final short COUNTER_HIGH_MASK = 0x0780; // 4 bits of 1s + 7 bits of 0s
    private static final short COUNTER_HIGH_SHIFT = 53;

    private static final short MACHINE_HIGH_MASK = 0x0fe0; // 7 bits of 1s + 5 bits of 0s
    private static final short MACHINE_HIGH_SHIFT = 48;

    private static final short COUNTER_LOW_MASK = 0x007f; // 7 bits of 1s
    private static final short COUNTER_LOW_SHIFT = 5;

    private static final short MACHINE_LOW_MASK = 0x001f; // 5 bits of 1s
    private static final short MACHINE_LOW_SHIFT = 0;
    ...
    public Snowflake(short machineId) {
        this.machineId = (machineId & (MACHINE_HIGH_MASK | MACHINE_LOW_MASK)); //take the first 12 bits, then shift it over appropriately
        /*
    ...
    private long buildUUID(long timestamp, long count) {
        long uuid = (count & COUNTER_HIGH_MASK) << COUNTER_HIGH_SHIFT;
        uuid |= (machineId & MACHINE_HIGH_MASK) << MACHINE_HIGH_SHIFT;
        uuid |= (timestamp & TIMESTAMP_MASK)    << TIMESTAMP_SHIFT;
        uuid |= (count     & COUNTER_LOW_MASK)  << COUNTER_LOW_SHIFT;
        uuid |= (machineId & MACHINE_LOW_MASK)  << MACHINE_LOW_SHIFT;
        return uuid;
    }
