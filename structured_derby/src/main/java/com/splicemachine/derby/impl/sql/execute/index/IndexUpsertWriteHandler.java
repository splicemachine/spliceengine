...
public class IndexUpsertWriteHandler extends AbstractIndexWriteHandler {
    private static final Logger LOG = Logger.getLogger(IndexUpsertWriteHandler.class);
    ...
    private KVPair update(KVPair mutation, WriteContext ctx) {
        ...
        try{
            ...
            LOG.error("Deleting key = " + BytesUtil.toHex(existingIndexRowKey));
            LOG.error("     old key = " + BytesUtil.toHex(mutation.getRow()));
            ...
            LOG.error("new key = " + BytesUtil.toHex(newIndexRowKey));
            LOG.error("new val = " + BytesUtil.toHex(indexValue));
            if(keepState)
