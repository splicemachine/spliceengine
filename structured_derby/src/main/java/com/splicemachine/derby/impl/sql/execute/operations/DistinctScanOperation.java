...
public class DistinctScanOperation extends ScanOperation implements SinkingOperation {
    ...
    public void init(SpliceOperationContext context) throws StandardException {
        super.init(context);
        FormatableArrayHolder fah = (FormatableArrayHolder) activation.getPreparedStatement().getSavedObject
            (hashKeyItem);
        ...
        for (int index = 0; index < fihArray.length; index++) {
            keyColumns[index] = FormatableBitSetUtils.currentRowPositionFromBaseRow(scanInformation
                                                                                        .getAccessedColumns(),
                                                                                    fihArray[index].getInt());
        }
    ...
    public SpliceNoPutResultSet executeScan(SpliceRuntimeContext runtimeContext) throws StandardException {
        ...
        RowProvider provider = getReduceRowProvider(this, OperationUtils.getPairDecoder(this, runtimeContext),
                                                    runtimeContext, true);
    ...
    public ExecRow getNextSinkRow(SpliceRuntimeContext spliceRuntimeContext) throws StandardException, IOException {
        if (sinkIterator == null) {
            ...
            buffer = new DistinctSortAggregateBuffer(SpliceConstants.ringBufferSize, null, supplier,
                                                     spliceRuntimeContext);
            ScannerIterator source = new ScannerIterator(regionScanner, getExecRowDefinition(),
                                                         operationInformation.getBaseColumnMap(), scanInformation,
                                                         descColumns);
    ...
    protected PairDecoder getTempDecoder() throws StandardException {
        PairDecoder decoder;
        KeyDecoder actualKeyDecoder = new KeyDecoder(BareKeyHash.decoder(keyColumns, null), 9);
        ExecRow templateRow = getExecRowDefinition();
        KeyHashDecoder actualRowDecoder = BareKeyHash.decoder(IntArrays.complement(keyColumns,
                                                                                   templateRow.nColumns()), null);
    ...
    public KeyEncoder getKeyEncoder(SpliceRuntimeContext spliceRuntimeContext) throws StandardException {
        HashPrefix keyPrefix = new BucketingPrefix(new FixedPrefix(uniqueSequenceID), HashFunctions.murmur3(0),
    ...
    public RowProvider getMapRowProvider(SpliceOperation top, PairDecoder decoder,
        ...
        try {
            Scan reduceScan = Scans.buildPrefixRangeScan(uniqueSequenceID, SpliceUtils.NA_TRANSACTION_ID);
                        /*
						 ...
						 *
						 * So there's no point in actually sending all this information over to the server,
						 * since it won't be doing anything
						 * anyway.
						 *
            ...
            return new DistributedClientScanProvider("distinctScanReduce", SpliceConstants.TEMP_TABLE_BYTES, reduceScan,
    ...
    public RowProvider getReduceRowProvider(SpliceOperation top, PairDecoder decoder,
                                            ...
                                            boolean returnDefaultValue) throws StandardException {
    ...
    protected JobResults doShuffle(SpliceRuntimeContext runtimeContext) throws StandardException {
        ...
        RowProvider provider = new ClientScanProvider("shuffle", Bytes.toBytes(Long.toString(scanInformation
                                                                                                 .getConglomerateId()
        )), scan, null, runtimeContext);
    ...
    private class ScannerIterator implements StandardIterator<ExecRow> {
        ...
        public ExecRow next(SpliceRuntimeContext spliceRuntimeContext) throws StandardException, IOException {
            ...
            do {
                ...
                if (scanInformation.getAccessedPkColumns() != null && scanInformation.getAccessedPkColumns()
                                                                                     .getNumBitsSet() > 0) {
        ...
        private EntryPredicateFilter getPredicateFilter(SpliceRuntimeContext spliceRuntimeContext) throws
            StandardException, IOException {
            if (!cachedPredicateFilter) {
                Scan scan = getScan(spliceRuntimeContext);
                predicateFilter = EntryPredicateFilter.fromBytes(scan.getAttribute(SpliceConstants
                                                                                       .ENTRY_PREDICATE_LABEL));
