...
public class SpliceTransactionManager implements XATransactionController, TransactionManager {
	private static Logger LOG = Logger.getLogger(SpliceTransactionManager.class);
	/**
	...
	private final void init(
			SpliceAccessManager myaccessmanager, 
			Transaction      theRawTran,
			SpliceTransactionManager   parent_tran)
	{
	...
	protected SpliceTransactionManager(
			SpliceAccessManager myaccessmanager, 
			Transaction      theRawTran,
			SpliceTransactionManager   parent_transaction)
					...
					throws StandardException
	{
		...
		this.context.setTransactionManager(this);
	...
	public void addColumnToConglomerate(
			...
			int         collation_id)
					...
					{
		if (LOG.isTraceEnabled())
			LOG.trace("addColumnToConglomerate conglomID " + conglomId + ", column_id" + column_id + ", template_column " + template_column);
	...
	public long createConglomerate(
		...
		if (LOG.isTraceEnabled())
			LOG.trace("createConglomerate: implementation " + implementation + ", template " + template + ", properties " + properties);
	...
	public long createAndLoadConglomerate(
			...
			long[] rowCount)
					...
					{
		if (LOG.isTraceEnabled())
			LOG.trace("createAndLoadConglomerate: implementation " + implementation + ", template " + template + ", properties " + properties);
	...
	{
		if (LOG.isTraceEnabled())
			LOG.trace("recreateAndLoadConglomerate: implementation " + implementation + ", template " + template + ", properties " + properties);
	...
	public long loadConglomerate(
			...
			RowLocationRetRowSource rowSource)
					...
					{
		if (LOG.isTraceEnabled())
			LOG.trace("loadConglomerate conglomId " + conglomId + ", rowSource " + rowSource);
		// Find the conglomerate.
		Conglomerate conglom = findExistingConglomerate(conglomId);
	...
	public ConglomerateController openCompiledConglomerate(
			...
			DynamicCompiledOpenConglomInfo  dynamic_info)
					...
					{
		if (LOG.isTraceEnabled())
			LOG.trace("openCompiledConglomerate static_info " + static_info + ", dynamic_info  " + dynamic_info);
		if (SanityManager.DEBUG)
		{
	...
	public BackingStoreHashtable createBackingStoreHashtableFromScan(
			...
			boolean                 keepAfterCommit)
					throws StandardException {
		if (LOG.isTraceEnabled())
			LOG.trace("createBackingStoreHashtableFromScan conglomId " + conglomId);
		return (
	...
	public Serializable getProperty(String key) throws StandardException {
		...
		return (accessmanager.getTransactionalProperties().getProperty(this, key));
	}
	...
	public Serializable getPropertyDefault(String key) throws StandardException {
		...
		return (accessmanager.getTransactionalProperties().getPropertyDefault(this, key));
	}
	...
	public SortCostController openSortCostController(
			Properties implParameters) throws StandardException {
		if (LOG.isTraceEnabled())
	...
	public ScanController openSortScan(
			long id, boolean hold) throws StandardException {
		Sort sort;
		if (LOG.isTraceEnabled())
	...
	public boolean checkVersion(
		...
		return(accessmanager.getRawStore().checkVersion(
	...
	public TransactionManager getInternalTransaction()
		...
		SpliceTransactionManager rt   = new SpliceTransactionManager(accessmanager, rawtran, null);
		SpliceTransactionManagerContext rtc   = 
				new SpliceTransactionManagerContext(cm, AccessFactoryGlobals.RAMXACT_INTERNAL_CONTEXT_ID, rt, true /*abortAll */);
	...
	public TransactionController startNestedUserTransaction(
		...
		SpliceTransactionManagerContext rtc   = 
				new SpliceTransactionManagerContext(cm, AccessFactoryGlobals.RAMXACT_CHILD_CONTEXT_ID, rt, true /*abortAll */);
	...
	/*public String getActiveStateTxIdString(String newTransID)
	{
		if (LOG.isTraceEnabled())
	...
	public SpliceScan openTempScan(boolean b, int i, int lockMode,
			int isolationLevel, FormatableBitSet accessedCols,
			DataValueDescriptor[] scanPosition, int ge, Object object,
			DataValueDescriptor[] scanPosition2, int gt) {
		// TODO Auto-generated method stub
		return null;
	}
	public ScanController openTempScan(
		    boolean                         hold,
		    int                             open_mode,
		    int                             lock_level,
		    int                             isolation_level,
    	...
    	if (LOG.isTraceEnabled())
    		LOG.trace("openTempScan");			
		SpliceScan spliceScan = new SpliceScan(scanColumnList,startKeyValue,startSearchOperator,qualifier,stopKeyValue,stopSearchOperator,this.getRawStoreXact(),false);
		scanControllers.add(spliceScan);
		return spliceScan;
	}
