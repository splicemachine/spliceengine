...
public class SpliceTransactionManager implements XATransactionController,
	...
	private final void init(SpliceAccessManager myaccessmanager,
	...
	SpliceTransactionManager(SpliceAccessManager myaccessmanager,
			SpliceTransactionManager tc, int format_id, byte[] global_id,
		...
		this.rawtran.createXATransactionFromLocalTransaction(format_id,
				global_id, branch_id);
	...
	private ConglomerateController openConglomerate(Conglomerate conglom,
		...
		ConglomerateController cc = conglom.open(this, rawtran, hold,
	...
	public long createConglomerate(String implementation,
		...
		if (LOG.isTraceEnabled())
			LOG.trace("createConglomerate: implementation " + implementation
	...
	public long createAndLoadConglomerate(String implementation,
			...
			throws StandardException {
		if (LOG.isTraceEnabled())
			LOG.trace("createAndLoadConglomerate: implementation "
	...
	{
		if (LOG.isTraceEnabled())
			LOG.trace("recreateAndLoadConglomerate: implementation "
					+ implementation + ", template " + template
					+ ", properties " + properties);
	...
	public String debugOpened() throws StandardException {
		...
		if (SanityManager.DEBUG) {
			...
			for (Iterator it = conglomerateControllers.iterator(); it.hasNext();) {
			...
			if (tempCongloms != null) {
				for (Iterator it = tempCongloms.keySet().iterator(); it
						.hasNext();) {
	...
	public GroupFetchScanController openGroupFetchScan(long conglomId,
		...
		ScanManager sm = conglom.openScan(this, rawtran, hold, open_mode,
				...
				isolation_level, scanColumnList, startKeyValue,
				startSearchOperator, qualifier, stopKeyValue,
				stopSearchOperator, (StaticCompiledOpenConglomInfo) null,
				(DynamicCompiledOpenConglomInfo) null);
	...
	public ScanController openCompiledScan(boolean hold, int open_mode,
		...
		if (SanityManager.DEBUG) {
			...
			SanityManager
					.ASSERT(dynamic_info instanceof DynamicCompiledOpenConglomInfo);
		}
	...
	public void commit() throws StandardException {
		...
		// if
		// (SpliceTransactionFactory.USER_CONTEXT_ID.equals(((SpliceTransaction)rawtran).getContext().getIdName()))
		// abort();
	...
	public ScanController openTempScan(boolean hold, int open_mode,
			int lock_level, int isolation_level,
			FormatableBitSet scanColumnList,
			DataValueDescriptor[] startKeyValue, int startSearchOperator,
			Qualifier qualifier[][], DataValueDescriptor[] stopKeyValue,
		...
		SpliceScan spliceScan = new SpliceScan(scanColumnList, startKeyValue,
				startSearchOperator, qualifier, stopKeyValue,
				stopSearchOperator, this.getRawStoreXact(), false);
