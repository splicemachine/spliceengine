    ...
    public RowEncoder getRowEncoder(SpliceRuntimeContext spliceRuntimeContext) throws StandardException {
        return RowEncoder.create(sourceExecIndexRow.nColumns(), convertIntegers(allKeyColumns),convertBooleans(groupByDescAscInfo), null, new KeyMarshall() {
            ...
            public void encodeKey(DataValueDescriptor[] columns,
                 ...
                 */
                byte hash = (byte)(MurmurHash.murmur3_32(currentKey) & 0x000000ff);
                byte[] key = BytesUtil.concatenate(hash,uniqueSequenceID,currentKey,SpliceUtils.getUniqueKey());
                /*
            ...
            public void decode(DataValueDescriptor[] columns,
                ...
                KeyType.BARE.decode(columns, reversedKeyColumns, sortOrder, rowDecoder);
            }
        ...
        if(aggregator==null){
            StandardSupplier<ExecRow> emptyRowSupplier = new StandardSupplier<ExecRow>() {
                ...
                public ExecRow get() throws StandardException {
                    return getActivation().getExecutionFactory().getIndexableRow(rowAllocator.invoke());
                }
            };
            ...
            AggregateBuffer buffer = new AggregateBuffer(SpliceConstants.ringBufferSize,
                    aggregates,isTemp,emptyRowSupplier,collector,false);
            ...
            int[] groupColumns = new int[groupByColumns.size()];
            boolean [] sortByColumns = new boolean[groupByColumns.size()];
            for(int i=0;i<groupByColumns.size();i++){
                groupColumns[i] = groupByColumns.get(i);
                sortByColumns[i] = groupByDescAscInfo.get(i);
            }
            aggregator = new GroupedAggregator(buffer,sourceIterator,
                    groupColumns,sortByColumns,isRollup);
        }
...
//        ExecRow row = doSinkAggregation(spliceRuntimeContext);
    ...
    public ExecRow nextRow(final SpliceRuntimeContext spliceRuntimeContext) throws StandardException, IOException {
        if(aggregator==null){
             ...
             * in memory.
             */
            AggregateBuffer buffer = new AggregateBuffer(1, aggregates,isTemp,emptyRowSupplier,collector,true);
            int[] groupColumns = new int[groupByColumns.size()];
            boolean [] sortByColumns = new boolean[groupByColumns.size()];
            for(int i=0;i<groupByColumns.size();i++){
                groupColumns[i] = groupByColumns.get(i);
            ...
            SpliceResultScanner scanner = getResultScanner(groupColumns,spliceRuntimeContext);
...
//        if (LOG.isTraceEnabled())
//            SpliceLogUtils.trace(LOG, "next %s",row);
//        return row;
    }
...
//
//        Pair<ByteBuffer,ExecRow> next = hbs.getNextAggregatedRow();
//        ExecRow rowResult = null;
//        if(next != null){
//            makeCurrent(next.getFirst(),next.getSecond());
//            rowResult = next.getSecond();
//        }else{
