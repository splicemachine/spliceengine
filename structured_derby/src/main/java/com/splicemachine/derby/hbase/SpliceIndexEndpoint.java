...
public class SpliceIndexEndpoint extends BaseEndpointCoprocessor implements BatchProtocol{
    ...
    private static volatile SpliceWriteControl control = new SpliceWriteControl((SpliceConstants.ipcThreads-taskWorkers-ipcReserved)/2,(SpliceConstants.ipcThreads-taskWorkers-ipcReserved)/2,SpliceConstants.maxDependentWrites,SpliceConstants.maxIndependentWrites);
    ...
    public BulkWritesResult bulkWrite(BulkWrites bulkWrites) throws IOException {
		...
		SharedCallBuffer indexSharedCallBuffer = new IndexSharedCallBuffer();
		boolean dependent = isDependent();
		SpliceWriteControl.Status status = null;
		int kvPairSize = (int) bulkWrites.getKVPairSize();
		try {
			status = (dependent)?control.performDependentWrite(kvPairSize):control.performIndependentWrite(kvPairSize);
			switch(status) {
				case REJECTED:
			...
			for (int i = 0; i< size; i++) {
	    		...
	    		if (endpoint == null) {
	    			...
	    			startPoints.add(Pair.newPair(new BulkWriteResult(new WriteResult(Code.NOT_SERVING_REGION, String.format("endpoint not found for region %s on region %s",bulkWrite.getEncodedStringName(), rce.getRegion().getRegionNameAsString()))), endpoint));
		    			...
		    			startPoints.add(Pair.newPair(sendUpstream(bulkWrite,endpoint,indexSharedCallBuffer), endpoint));
	    			}	    			
	    	}
		...
		return result;
			...
			case REJECTED:
				break;
			case DEPENDENT:
				control.finishDependentWrite(kvPairSize);
				break;
			case INDEPENDENT:
				control.finishIndependentWrite(kvPairSize);						
				break;
		}
		}
    }
    
    private static BulkWriteResult sendUpstream(BulkWrite bulkWrite, SpliceIndexEndpoint endpoint, SharedCallBuffer indexSharedCallBuffer) throws IOException {
		...
		try{
			...
			try {
					context = endpoint.getWriteContext(indexSharedCallBuffer,bulkWrite.getTxn(),endpoint.region,endpoint.rce,bulkWrite.getMutations().size());
    ...
    private static BulkWriteResult flushAndClose(BulkWriteResult writeResult, BulkWrite bulkWrite,SpliceIndexEndpoint endpoint) throws IOException {
		...
		private WriteContext getWriteContext(SharedCallBuffer indexSharedCallBuffer, TxnView txn, TransactionalRegion region, RegionCoprocessorEnvironment rce,int writeSize) throws IOException, InterruptedException {
        Pair<LocalWriteContextFactory, AtomicInteger> ctxFactoryPair = getContextPair(conglomId);
        return ctxFactoryPair.getFirst().create(indexSharedCallBuffer,txn,region,rce);
    }
    ...
    public static void registerJMX(MBeanServer mbs) throws MalformedObjectNameException, NotCompliantMBeanException, InstanceAlreadyExistsException, MBeanRegistrationException {
		...
		public static class ActiveWriteHandlers implements ActiveWriteHandlersIface {
				...
				@Override public int getDependentWriteThreads() { return control.getWriteStatus().get().dependentWriteThreads; }
				@Override public int getIndependentWriteThreads() { return control.getWriteStatus().get().independentWriteThreads; }
				@Override public int getDependentWriteCount() {return control.getWriteStatus().get().dependentWriteCount; }
				@Override public int getIndependentWriteCount() { return control.getWriteStatus().get().independentWriteCount; }
