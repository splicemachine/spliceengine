    ...
    public void testGetTables() {
        ...
        try{
            ...
            rs = s.executeQuery("SELECT CAST ('' AS VARCHAR(128)) AS TABLE_CAT, SCHEMANAME AS TABLE_SCHEM,  TABLENAME AS TABLE_NAME, " +
            		...
            		"WHERE (TTABBREV=TABLETYPE 	AND (SYS.SYSTABLES.SCHEMAID = SYS.SYSSCHEMAS.SCHEMAID) AND ((1=1) OR ? IS NOT NULL) " +
            		"AND (SYS.SYSSCHEMAS.SCHEMANAME LIKE '%APP%') " +
            		"AND (TABLENAME LIKE '%') AND TABLETYPE IN (?, ?, ?, ?)) ORDER BY TABLE_TYPE, TABLE_SCHEM, TABLE_NAME");
            while(rs.next()){
                LOG.info(String.format("1=%s",rs.getString(1)));
            }
        }catch (Exception e) {
