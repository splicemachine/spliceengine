...
public class InnerJoinTest extends BaseJoinTest {
	...
	public void testScrollableInnerJoin() throws SQLException {
		ResultSet rs = rule.executeQuery("select c.si, d.si from c inner join d on c.si = d.si");
		int j = 0;
		while (rs.next()) {
			j++;
	...
	public void testSinkableInnerJoin() throws SQLException {			
		ResultSet rs = rule.executeQuery("select c.si, count(*) from c inner join d on c.si = d.si group by c.si");
		int j = 0;
		while (rs.next()) {
			j++;
