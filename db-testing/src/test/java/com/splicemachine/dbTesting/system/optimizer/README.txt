...
A default set of queries are available to run (classes com.splicemachine.dbTesting.system.langtest.
query.Query1 - Query6):
...
The test settings are located on the com.splicemachine.dbTesting.system.optimizer.StaticValues
class. The test creates 64 tables with different column types and populates them with 
the a specified number of rows (com.splicemachine.dbTesting.system.langtest.StaticValues.NUM_OF_ROWS).

PACKAGING:
==========
The test resides under the com.splicemachine.dbTesting.system.optimizer package. The main
class to invoke is com.splicemachine.dbTesting.system.optimizer.RunOptimizerTest. All the
variable settings used in the test are set in the com.splicemachine.dbTesting.system.optimizer.StaticValues
