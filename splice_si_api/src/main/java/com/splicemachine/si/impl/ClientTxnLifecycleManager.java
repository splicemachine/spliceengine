...
public class ClientTxnLifecycleManager implements TxnLifecycleManager {
		...
		@ThreadSafe private final TimestampSource timestampSource = SIDriver.getTimestampSource();
		@ThreadSafe private TxnStore store = SIDriver.getTxnStore();
		@ThreadSafe private KeepAliveScheduler keepAliveScheduler = SIDriver.getKeepAliveScheduler();
        @ThreadSafe private SExceptionLib exceptionLib = SIDriver.getExceptionLib();
        private volatile boolean restoreMode = false;

		public ClientTxnLifecycleManager() {
		}
    ...
    private static final Logger LOG = Logger.getLogger(ClientTxnLifecycleManager.class);
		...
		public Txn beginChildTransaction(TxnView parentTxn,
				...
				if(destinationTable!=null && !parentTxn.allowsWrites())
						throw exceptionLib.getDoNotRetryIOException("Cannot create a writable child of a read-only transaction. Elevate the parent transaction("+parentTxn.getTxnId()+") first");
        if(parentTxn.getState()!= Txn.State.ACTIVE)
            throw exceptionLib.getDoNotRetryIOException("Cannot create a child of an inactive transaction. Parent: "+ parentTxn);
        ...
        if(destinationTable!=null){
            ...
            if(!txnToCommit.allowsWrites())
                throw exceptionLib.getDoNotRetryIOException("Cannot chain a writable transaction from a read-only transaction. Elevate the transaction("+txnToCommit.getTxnId()+") first");
        }
        ...
        if(!txnToCommit.allowsWrites() && Txn.ROOT_TRANSACTION.equals(parentTxn)){
             ...
             */
            throw exceptionLib.getDoNotRetryIOException("Cannot chain a read-only parent transaction from a read-only transaction. Elevate the transaction("+txnToCommit.getTxnId()+") first");
        }
        txnToCommit.commit();
