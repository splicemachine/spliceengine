...
/**
 * Transaction filter which performs basic transactional filtering for a redo log model
 *
 */
public class SimpleActiveTxnFilter implements TxnFilter{
    ...
    protected UnsafeRecord unsafeRecord = new UnsafeRecord();
    ...
    public SimpleActiveTxnFilter(TxnView myTxn,
    ...
    private void readResolve(DataCell element) throws IOException{
		 ...
		 */
        long ts=unsafeRecord.getTxnId1();//dataStore.getOpFactory().getTimestamp(element);
    ...
    protected boolean isVisible(long txnId) throws IOException{
    ...
    protected TxnView fetchTransaction(long txnId) throws IOException{
    ...
    private void ensureTransactionIsCached(DataCell data) throws IOException{
        if (!unsafeRecord.isResolved())
            return;
        long txnId=unsafeRecord.getTxnId1();//this.dataStore.getOpFactory().getTimestamp(data);
