...
public class RedoTransactor implements Transactor{
    ...
    public MutationStatus[] processPutBatch(Partition table,RollForward rollForwardQueue,DataPut[] mutations,ExecRow execRow) throws IOException{
        ...
        for (int i = 0; i< mutations.length; i++) {
            if (execRow == null)
                execRow = (ExecRow) SerializationUtils.deserialize(mutations[0].getAttribute(SIConstants.SI_EXEC_ROW));
    ...
    */
    ...
    private IntObjectOpenHashMap<DataPut> checkConflictsForKvBatch(Partition table,
        ...
        if (possibleConflicts != null) {
            ...
            resolvePossibleConflictsToActual(table,transaction,txns,possibleConflicts,dataAndLocks);
    ...
    public void resolvePossibleConflictsToActual(Partition table, TxnView transaction, TxnView[] txns, DataResult[] possibleConflicts,
                                                         Pair<KVPair, Lock>[] dataAndLocks) throws IOException {
        for (int i = 0; i< possibleConflicts.length; i++) {
            if (txns[i] != null) {
                if (txns[i].getEffectiveState() != Txn.State.ROLLEDBACK)
                    continue; // All Good, pass on for Redo Record
                else {
                    if (possibleConflicts[i].activeData().version() == 1) {
                        possibleConflicts[i] = null; // Overwrite it...
                        continue;
                    }
                    else {
                        DataResult dr = possibleConflicts[i];
                        long timestamp = dr.activeData().version();
                        while (true) {
                            DataGet dataGet = opFactory.newGet(dataAndLocks[i].getFirst().getRowKey(),null);
                            dataGet.addColumn(SIConstants.DEFAULT_FAMILY_REDO_BYTES,SIConstants.PACKED_COLUMN_BYTES);
                            dataGet.setTimeRange(timestamp-1,timestamp);
                            dr = table.get(dataGet,null);
                            if (dr == null || dr.size() == 0) {
                                possibleConflicts[i] = null;
                                break;
                            }
                            timestamp = dr.redoData().version();
                            // Throw Exception if Missing...
                            TxnView[] fetchedTransactions = fetchTransactions(new DataResult[]{dr}, txnSupplier);
                            if (fetchedTransactions[0].getEffectiveState() != Txn.State.ROLLEDBACK ) { // As Long as not rolled back, roll
                                possibleConflicts[i] = dr;
                                break;
                            }
                            if (dr.redoData().version() == 1) {
                                possibleConflicts[i] = null; // Overwrite it...
                                    break;
                            }
                        }
        ...
        try {
            ...
            if (kvPair.getType() == KVPair.Type.DELETE || kvPairRecord.hasTombstone()) {
                if (possibleLegacyRecord == null) {
                    ...
                    printoutPair(kvPair,execRow,"Delete with record to delete");
                    kvPairRecord.setHasTombstone(true);
