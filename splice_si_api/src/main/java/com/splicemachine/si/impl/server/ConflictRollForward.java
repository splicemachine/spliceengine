...
public class ConflictRollForward {
    ...
    public void handle(DataCell cell) throws IOException {
        ...
        if (txn.getEffectiveState().equals(Txn.State.COMMITTED)) {
            ...
            delete.deleteColumn(SIConstants.DEFAULT_FAMILY_BYTES,SIConstants.TOMBSTONE_COLUMN_BYTES, cell.version()); //delete all the columns for our family only
            delete.deleteColumn(SIConstants.DEFAULT_FAMILY_BYTES,SIConstants.ANTI_TOMBSTONE_VALUE_BYTES, cell.version()); //delete all the columns for our family only
