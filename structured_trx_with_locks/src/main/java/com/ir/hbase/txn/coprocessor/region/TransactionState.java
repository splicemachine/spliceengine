...
public class TransactionState {
	...
	public TransactionState(final String transactionID, HRegion region, RecoverableZooKeeper rzk, TxnLockManager lockManager, Map<String, TransactionState> transactionStateByID, HTablePool tablePool, String txnLogPath, boolean isRecovered) {
		...
		try {
			if (!isRecovered) { 
				if (LOG.isDebugEnabled())
					LOG.debug("Creating node " + TxnUtils.getTransactionalRegionPath(transactionID, regionId));
				path = rzk.create(TxnUtils.getTransactionalRegionPath(transactionID, regionId), Bytes.toBytes(TransactionStatus.PENDING.toString()), ZooDefs.Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT);
			...
			if (LOG.isDebugEnabled())
				LOG.debug("Given transaction ID was not found.");
	...
	private class TxnIdWatcher implements Watcher {
		...
		public void process(WatchedEvent event) {
			if (LOG.isDebugEnabled())
				LOG.debug("Transaction state node watcher is triggered, transaction ID " + transactionID + " in table " + region.getTableDesc().getNameAsString());
	...
	protected void prepareCommit() {
		if (LOG.isDebugEnabled())
			LOG.debug("Prepare commit, transaction ID " + transactionID);
		try {
			...
			e.printStackTrace();
			LOG.debug("Failed to log write action to prepare commit.");
			return;
		}
		setStatus(TransactionStatus.PREPARE_COMMIT);
		TxnLogger.createTxnLogNode(txnLogPath, regionId, rzk);
	}

	protected void doCommit() {
		if (LOG.isDebugEnabled())
			LOG.debug("Do commit, transaction ID " + transactionID);
		if (hasWrite()) {
			try {
				...
				e.printStackTrace();
				LOG.debug("Failed to perform write action to do commit.");
				return;
			}
		}
		setStatus(TransactionStatus.DO_COMMIT);
		lockManager.releaseExclusiveLocks(transactionID);
		TxnLogger.removeTxnLogNode(txnLogPath, regionId, rzk);
		retire();
	}
	...
	protected void executeAction(WriteAction action, HRegion region) throws IOException {
		...
		if (null != put) {
			...
			if (LOG.isDebugEnabled())
				LOG.debug("Execute put action with row key " + Bytes.toString(put.getRow()) + " on region " + regionId);
			region.put(put, true);
		}
		...
		if (null != delete) {
			...
			if (LOG.isDebugEnabled())
				LOG.debug("Execute delete action with row key " + Bytes.toString(delete.getRow()) + " on region " + regionId);
	...
	protected void abort() {
		LOG.debug("Abort, transaction ID " + transactionID);
		retire();
	}

	protected void retire() {
		if (LOG.isDebugEnabled())
			LOG.debug("Retire, transaction ID in region " + regionId + " " + path);
		/*try {
			rzk.delete(path, -1);
		} catch (InterruptedException e) {
			e.printStackTrace();
			...
			e.printStackTrace();
		}*/ //-gfan for single thread coordinator, not to delete node here. delete recursively instead.
		transactionStateByID.remove(transactionID);
	}

	public void addWrite(final Put write) {
		LOG.debug("Add write to transaction state with transaction ID " + transactionID);
	...
	public void addDelete(final Delete delete) {
		LOG.debug("Add delete to transaction state with transaction ID " + transactionID);
	...
	private KeyValue[] getAllKVs(final Scan scan) {
		LOG.debug("Get all KVs from write actions, transaction ID " + transactionID);
	...
	protected void setStatus(TransactionStatus status) {
		if (LOG.isDebugEnabled())
			LOG.debug("Set status to " + status.toString() + ", transaction ID " + transactionID + " in table " + region.getTableDesc().getNameAsString());
		if (nodeExist()) {
			try {
