...
public class PartitionWriteHandler implements WriteHandler {
    ...
    protected List<Record> mutations = Lists.newArrayList();
    ...
    @Override
    public void next(Record kvPair, WriteContext ctx) {
        /*
        ...
        else if(!region.keyInRange(kvPair.rowKeySlice()))
            ctx.failed(kvPair, WriteResult.wrongRegion());
        else {
            if (kvPair.getRecordType() == RecordType.CANCEL){
                mutations.add(kvPair.cancelToDelete());
            }
            else
    ...
    public void flush(final WriteContext ctx) throws IOException {
        ...
        Collection<Record> filteredMutations = Collections2.filter(mutations, new Predicate<Record>() {
            @Override
        ...
        try {
            ...
            if(t instanceof WriteConflict){
                ...
                for(Record mutation : filteredMutations){
    ...
    private void doWrite(WriteContext ctx, Collection<Record> toProcess) throws IOException {
        ...
        Iterator<Record> mutationIter = toProcess.iterator();
        while(statusIter.hasNext()){
            ...
            Record mutation = mutationIter.next();
            if(stat.isNotRun())
                ctx.notRun(mutation);
