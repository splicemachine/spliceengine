...
public class WriteCoordinator {
    ...
    public RecordingCallBuffer<Record> writeBuffer(Partition partition, Txn txn) {
        return writeBuffer(partition,txn, PipelineUtils.noOpFlushHook);
    }

    public RecordingCallBuffer<Record> synchronousWriteBuffer(Partition partition, Txn txn) {
        return synchronousWriteBuffer(partition,txn, PipelineUtils.noOpFlushHook,defaultWriteConfiguration);
    }

    public RecordingCallBuffer<Record> writeBuffer(Partition partition, Txn txn, PreFlushHook preFlushHook) {
        return writeBuffer(partition,txn,preFlushHook,Metrics.noOpMetricFactory());
    }

    public RecordingCallBuffer<Record> noIndexWriteBuffer(byte[] tableId, Txn txn, final MetricFactory metricFactory) throws IOException{
    ...
    public RecordingCallBuffer<Record> noIndexWriteBuffer(Partition partition, Txn txn, final MetricFactory metricFactory) {
    ...
    public RecordingCallBuffer<Record> writeBuffer(Partition partition, Txn txn, final PreFlushHook preFlushHook,final MetricFactory metricFactory) {
        WriteConfiguration config = defaultWriteConfiguration;
        monitor.outstandingBuffers.incrementAndGet();
        //if it isn't active, don't bother creating the extra object
        if (metricFactory.isActive()) {
    ...
    public RecordingCallBuffer<Record> writeBuffer(byte[] partition, Txn txn, PreFlushHook preFlushHook) throws IOException{
        return writeBuffer(partition, txn, preFlushHook, defaultWriteConfiguration,Metrics.noOpMetricFactory());
    }

    public RecordingCallBuffer<Record> writeBuffer(byte[] partition, Txn txn, MetricFactory metricsFactory) throws IOException{
        return writeBuffer(partition, txn, PipelineUtils.noOpFlushHook, defaultWriteConfiguration,metricsFactory);
    }

    public RecordingCallBuffer<Record> writeBuffer(byte[] partition,Txn txn,PreFlushHook noOpFlushHook,
    ...
    public RecordingCallBuffer<Record> writeBuffer(Partition partition, Txn txn, final int maxEntries) {
    ...
    public RecordingCallBuffer<Record> synchronousWriteBuffer(Partition partition,
    ...
    public RecordingCallBuffer<Record> writeBuffer(Partition table,Txn txn,WriteConfiguration writeConfiguration){
