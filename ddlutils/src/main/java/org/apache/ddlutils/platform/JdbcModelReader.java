...
public class JdbcModelReader {
     ...
     * @see TableType
     */
    public String[] _sysTableTypes = {"SYSTEM TABLE"};

    /**
     * All non system table types
     * @see TableType
     */
    public String[] _allNonSysTableTypes = {"TABLE", "VIEW", "GLOBAL TEMPORARY", "LOCAL TEMPORARY", "ALIAS", "SYNONYM"};

    /**
     * The table types to recognize per default - {@link #_allNonSysTableTypes all table types} - but can be set by caller.
     */
    ...
    public JdbcModelReader(Platform platform) {
        _defaultTableTypes = new String[_allNonSysTableTypes.length];

        System.arraycopy(_allNonSysTableTypes, 0, _defaultTableTypes, 0, _allNonSysTableTypes.length);
