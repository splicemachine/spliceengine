...
public class PlatformFactory {
     ...
     * @return The platform or <code>null</code> if the database is not supported
     */
    public static synchronized Platform createNewPlatformInstance(String jdbcDriver, String jdbcConnectionUrl) throws
        DdlUtilsException {
     ...
     * @return The platform or <code>null</code> if the database is not supported
     */
    public static synchronized Platform createNewPlatformInstance(DataSource dataSource, String username, String password)
    ...
    private static void registerPlatforms() {
        // TODO: JC - add these back in after resolving XML breakage due to adding Schema to model
...
//        addPlatform(_platforms, SybaseASE15Platform.DATABASENAME, SybaseASE15Platform.class);
        addPlatform(_platforms, CloudscapePlatform.DATABASENAME, CloudscapePlatform.class);
        addPlatform(_platforms, DerbyPlatform.DATABASENAME, DerbyPlatform.class);
        addPlatform(_platforms, SplicePlatform.DATABASENAME, SplicePlatform.class);
    }
    ...
    private static synchronized void addPlatform(Map<String, Class> platformMap, String platformName, Class platformClass) {
        if (!Platform.class.isAssignableFrom(platformClass)) {
            throw new IllegalArgumentException("Cannot register class " + platformClass.getName() + " because it does not " +
                                                   "implement the " + Platform.class.getName() + " interface");
        }
