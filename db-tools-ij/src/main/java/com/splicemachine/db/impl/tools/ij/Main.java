...
public class Main {
    ...
    public static void mainCore(String[] args, Main main)
        ...
        if (inputResourceName != null) {
            localizedInput = langUtil.getNewInput(util.getResourceAsStream(inputResourceName));
        } else if (file == null) {
            localizedInput = langUtil.getNewInput(System.in);
            out.flush();
        } else {
            try {
                ...
                if (inputStream != null) {
                    inputStream = new BufferedInputStream(inputStream, utilMain.BUFFEREDFILESIZE);
                    localizedInput = langUtil.getNewInput(inputStream);
                }
            } catch (PrivilegedActionException e) {
                if (Boolean.getBoolean("ij.searchClassPath")) {
                    localizedInput = langUtil.getNewInput(util.getResourceAsStream(file));
                }
        ...
        // Use the main parameter to get to a new Main that we can use.
        // (We can't do the work in Main(out) until after we do all the work above us in this method.
        Main me = main.getMain(out);

        // Let the processing begin!
        me.go(localizedInput, out);
        if (localizedInput != null) {
