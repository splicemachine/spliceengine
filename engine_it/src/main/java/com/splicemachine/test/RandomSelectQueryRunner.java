 ...
 *         Date: 9/16/14
 */
public class RandomSelectQueryRunner extends BaseQueryRunner{
    private String sql = "select * from %s where a = ?";
    ...
    protected BaseQueryRunner.Runner newRunner(Connection jdbcConn, int numIterations, File outputDir, int threadId, CountDownLatch startLatch) throws IOException {
        ...
        return new BaseQueryRunner.Runner(jdbcConn,numIterations,outputDir,threadId,startLatch) {
            @Override
    ...
    public static void main(String...args) throws Exception{
        new RandomSelectQueryRunner().run(args);
    }
}
