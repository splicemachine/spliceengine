...
//        String table=schemaWatcher.schemaName+"."+createTable(conn1,1);
        ...
        try(Statement s = conn1.createStatement()){
            Assert.assertEquals("Data is mysteriously present!",0l,conn1.count(s,"select * from " + table));
        }

        try(Statement s = conn2.createStatement()){
            Assert.assertEquals("Data is mysteriously present!",0l,conn2.count(s,"select * from " + table));
