...
public class MemStoreFlushAwareScanner extends StoreScanner {
	 protected static final Logger LOG = Logger.getLogger(MemStoreFlushAwareScanner.class);
	   ...
	   SDataLib dataLib = HTransactorFactory.getTransactor().getDataLib();
		...
		public boolean internalNext(List<KeyValue> outResult) throws IOException {
			if (LOG.isTraceEnabled())
				SpliceLogUtils.trace(LOG, "next kv=%s",outResult);
			if (beginRow) {
				beginRow = false;
				return outResult.add((KeyValue)MRConstants.MEMSTORE_BEGIN);
			}
			if (endRowNeedsToBeReturned) {
				endRowAlreadyReturned = true;
				return outResult.add((KeyValue)MRConstants.MEMSTORE_END);
			}
			if (didWeFlush()) {
				if (flushAlreadyReturned) {
					if (LOG.isTraceEnabled())
					...
					counter++;
					outResult.add((KeyValue)dataLib.newValue(Bytes.toBytes(counter),MRConstants.FLUSH,MRConstants.FLUSH,Long.MAX_VALUE,MRConstants.FLUSH));
				} else {
					if (LOG.isTraceEnabled())
						SpliceLogUtils.trace(LOG, "Flush has not returned");
						flushAlreadyReturned = true;
						outResult.add((KeyValue)MRConstants.MEMSTORE_BEGIN_FLUSH);
				}
				return true;
			}
	...
	boolean directInternalNext(List<KeyValue> result) throws IOException {
