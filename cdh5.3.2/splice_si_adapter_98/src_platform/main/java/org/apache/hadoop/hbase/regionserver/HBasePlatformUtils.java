...
public class HBasePlatformUtils{
	public static NavigableSet<Cell> getKvset(HStore store) {
		return new NavigableCastingSet<>(((DefaultMemStore)store.memstore).kvset,KeyValue.COMPARATOR);
	}

	public static NavigableSet<Cell> getSnapshot(HStore store) {
		return new NavigableCastingSet<>(((DefaultMemStore)store.memstore).snapshot,KeyValue.COMPARATOR);
	}
