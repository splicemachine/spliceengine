...
class ConnectionManager(url: String) extends Serializable {
  ...
  def getConnection(schemaTableName: String): (Connection, JDBCOptions) = if( autoCommitting ) {
    createConnection(schemaTableName)
  } else {
    (con.get, getJdbcOptions(schemaTableName))
  }
  
  private def createConnection(schemaTableName: String): (Connection, JDBCOptions) = {
    val jdbcOptions = getJdbcOptions( schemaTableName )
